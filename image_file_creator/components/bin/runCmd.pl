...
{
### For Android N 
    $FILE_CONTEXTS_PATH ="$package_path/root/file_contexts.bin";
    if (-e $FILE_CONTEXTS_PATH)
    {
        $MAKE_EXT4FS  = "$tmp_path/../../../android/out/host/linux-x86/bin/make_ext4fs";
        $SIMG2IMG     = "$tmp_path/../../../android/out/host/linux-x86/bin/simg2img";
        $E2FSCK_PATH  = "$tmp_path/../../../android/out/host/linux-x86/bin/e2fsck";
...
#        system("rm -rf $(CURDIR)/$(EXT4FS_SYSTEMIMG); sync"); 
	...
	{
	    system("$MAKE_EXT4FS -l $size_in_mb"."M -s -L $label_name -a $label_name $tmp_path/s.bin $package_path/$partitions_by_labels{$label_name}{\"label\"}; sync;");
	}
	elsif (($package eq "omv") && ($label_name eq "xen"))
	{
	    system("$MAKE_EXT4FS -l $size_in_mb"."M -s -L $label_name $tmp_path/s.bin $package_path/$partitions_by_labels{$label_name}{\"label\"}; sync;");
	}
	else
	{       
	    system("$MAKE_EXT4FS -l $size_in_mb"."M -s -L $label_name $USED_FILE_CONTEXTS_PATH -a $label_name $tmp_path/s.bin $package_path/$partitions_by_labels{$label_name}{\"label\"}; sync;");
	}
        my ($path, $ext4_img) = split /\//, $partitions_by_labels{$label_name}{"image_name"};
