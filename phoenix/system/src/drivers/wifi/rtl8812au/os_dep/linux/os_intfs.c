...
void rtw_napi_add(struct dvobj_priv *dvobj, u8 add)
{
        int i=0;
        if(add)
            RTW_INFO("netif_napi_add weight=%d\n", RTL_NAPI_WEIGHT);
        else
            RTW_INFO("net_napi_del\n");

        for (i = 0; i < dvobj->iface_nums; i++) {
                if (i >= IFACE_ID_MAX) {
                    RTW_INFO("%s %d >= IFACE_ID_MAX\n", __func__, i);
                        rtw_warn_on(1);
                        continue;
                }
...
int _netdev_open(struct net_device *pnetdev)
{
	...
	if (padapter->bup == _FALSE) {
        ...
        {
            RTW_INFO("_netdev_open: napi_enable\n");
...
netdev_open_error:
	...
	{
	    RTW_INFO("_netdev_open: napi_disable\n");
