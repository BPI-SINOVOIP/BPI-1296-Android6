...
u8	FwBuffer8192E[FW_SIZE_8192E];
...
)
{
	...
	DBG_871X(" ===> FirmwareDownload92E() fw source from %s.\n", (pFirmware->eFWSource ? "Header": "File"));

	switch(pFirmware->eFWSource)
	{
		case FW_SOURCE_IMG_FILE:
			#ifdef CONFIG_FILE_FWIMG
			rtStatus = rtw_retrive_from_file(fwfilepath, FwBuffer8192E, FW_SIZE_8192E);
			pFirmware->ulFwLength = rtStatus>=0?rtStatus:0;
			pFirmware->szFwBuffer = FwBuffer8192E;
			#endif //CONFIG_FILE_FWIMG
			break;
...
)
{
	...
	*/
	switch (GLNA_type) {
	case 2:
		pHalData->TypeGLNA = 3;  /* (14dB) */
		break;
	case 3:
		pHalData->TypeGLNA = 2;	/* (16dB) */
		break;
	case 4:
		pHalData->TypeGLNA = 1;	/* (18dB) */
		break;
	case 6:
	default:
		pHalData->TypeGLNA = 0;	/* (22dB) */
		break;
	}
