...
_func_enter_;
	...
	if(pdev->bus->number == 0x00) {
		...
		pmem_flags = 0;
		DBG_871X("RTD129X: PCIE SLOT1\n");
	} else if(pdev->bus->number == 0x01) {
		pmem_start = PCIE_SLOT2_MEM_START;
		pmem_len   = PCIE_SLOT2_MEM_LEN;
		pmem_flags = 0;
		DBG_871X("RTD129X: PCIE SLOT2\n");
	} else {
		DBG_871X(KERN_ERR "Wrong Slot Num\n");
	...
	if(pdev->bus->number == 0x00)
		dvobj->ctrl_start = (unsigned long)ioremap( PCIE_SLOT1_CTRL_START, 0x200);
	else if(pdev->bus->number == 0x01)
		dvobj->ctrl_start = (unsigned long)ioremap( PCIE_SLOT2_CTRL_START, 0x200);

	if(dvobj->ctrl_start == 0) {
		DBG_871X(KERN_ERR "RTD129X: Can't map CTRL mem\n");
		goto release_regions;
	}
