...
static int kbasep_vinstr_create_kctx(struct kbase_vinstr_context *vinstr_ctx)
{
    ...
    if (!xen_initial_domain())
        element = kzalloc(sizeof(*element), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
	    cli = kzalloc(sizeof(*cli), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        cli->accum_buffer = kzalloc(cli->dump_size, GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
        if (!xen_initial_domain())
            cli->dump_buffers_meta = kmalloc(tmp, GFP_KERNEL|GFP_DMA);
        else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        vinstr_ctx = kzalloc(sizeof(*vinstr_ctx), GFP_KERNEL|GFP_DMA);
    else
#endif
