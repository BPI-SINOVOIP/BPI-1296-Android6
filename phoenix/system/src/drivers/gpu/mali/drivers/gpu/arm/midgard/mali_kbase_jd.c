...
static int kbase_jd_pre_external_resources(struct kbase_jd_atom *katom, const struct base_jd_atom_v2 *user_atom)
{
    ...
    if (!xen_initial_domain())
	    katom->extres = kmalloc_array(katom->nr_extres, sizeof(*katom->extres), GFP_KERNEL | GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
	    kds_resources = kmalloc_array(katom->nr_extres, sizeof(struct kds_resource *), GFP_KERNEL|GFP_DMA);
    else
#endif
