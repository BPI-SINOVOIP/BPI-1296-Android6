...
struct kbase_gator_hwcnt_handles *kbase_gator_hwcnt_init(struct kbase_gator_hwcnt_info *in_out_info)
{
    ...
    if (!xen_initial_domain())
        hand = kzalloc(sizeof(*hand), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
	    hand->vinstr_buffer = kzalloc(dump_size, GFP_KERNEL|GFP_DMA);
    else
#endif
	...
	if (kbase_hw_has_feature(hand->kbdev, BASE_HW_FEATURE_V4)) {
        ...
        if (!xen_initial_domain())
            in_out_info->hwc_layout = kmalloc(sizeof(enum hwc_type) *
                    ...
                    in_out_info->nr_core_groups, GFP_KERNEL|GFP_DMA);
        else
#endif
		in_out_info->hwc_layout = kmalloc(sizeof(enum hwc_type) *
			MALI_MAX_NUM_BLOCKS_PER_GROUP *
...
#ifdef CONFIG_RTK_XEN_SUPPORT
        if (!xen_initial_domain())
            in_out_info->hwc_layout = kmalloc(sizeof(enum hwc_type) * (2 + nr_sc_bits + nr_l2), GFP_KERNEL|GFP_DMA);
        else
#endif
