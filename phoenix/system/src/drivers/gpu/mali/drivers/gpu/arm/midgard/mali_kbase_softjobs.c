...
static int kbase_debug_copy_prepare(struct kbase_jd_atom *katom)
{
    ...
    if (!xen_initial_domain())
	    buffers = kcalloc(nr, sizeof(*buffers), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
	    user_buffers = kmalloc_array(nr, sizeof(*user_buffers), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
	    info = kzalloc(sizeof(*info), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
	    ext_res = kzalloc(copy_size, GFP_KERNEL|GFP_DMA);
    else
#endif
