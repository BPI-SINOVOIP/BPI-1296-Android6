		...
		size_t start_offset)
{
    ...
    if (!xen_initial_domain())
			cache = kmalloc(sizeof(*cache), GFP_KERNEL|GFP_DMA);
    else
#endif
	...
	*flags &= ~BASE_MEM_SAME_VA;
    ...
    if (!xen_initial_domain())
        reg->gpu_alloc->imported.user_buf.pages = kmalloc_array(faulted_pages,
                sizeof(struct page *), GFP_KERNEL|GFP_DMA);
    else
#endif
	reg->gpu_alloc->imported.user_buf.pages = kmalloc_array(faulted_pages,
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        map = kzalloc(sizeof(*map), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        pages = kmalloc_array(page_count, sizeof(struct page *), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        va = dma_alloc_attrs(kctx->kbdev->dev, size, &dma_pa, GFP_KERNEL|GFP_DMA, &attrs);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        va = dma_alloc_writecombine(kctx->kbdev->dev, size, &dma_pa, GFP_KERNEL|GFP_DMA);
    else
#endif
