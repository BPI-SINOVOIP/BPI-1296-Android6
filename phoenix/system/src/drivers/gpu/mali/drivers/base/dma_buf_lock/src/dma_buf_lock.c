...
static int dma_buf_lock_dolock(dma_buf_lock_k_request *request)
{
    ...
    if (!xen_initial_domain())
        resource = kzalloc(sizeof(dma_buf_lock_resource), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        resource->list_of_dma_buf_fds = kmalloc(size, GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        resource->dma_bufs = kmalloc(size, GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        resource->kds_resources = kmalloc(size, GFP_KERNEL|GFP_DMA);
    else
#endif
