...
static int __init umpp_linux_initialize_module(void)
{
    ...
    if (!ump_ops) {
        ...
        if (!xen_initial_domain())
            ump_ops = kzalloc(sizeof(*ump_ops), GFP_KERNEL|GFP_DMA);
        else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
        if (!xen_initial_domain())
            tracker = kmalloc(sizeof(*tracker), GFP_KERNEL|GFP_DMA | __GFP_HARDWALL);
        else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        it = kmalloc(sizeof(*it), GFP_KERNEL|GFP_DMA);
    else
#endif
