...
{
#ifdef CONFIG_RTK_XEN_SUPPORT
	umpp_descriptor_mapping * map;
    if (!xen_initial_domain())
        map = kzalloc(sizeof(umpp_descriptor_mapping), GFP_KERNEL|GFP_DMA);
    else
        map = kzalloc(sizeof(umpp_descriptor_mapping), GFP_KERNEL);
#else
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        table = kzalloc(sizeof(umpp_descriptor_table) + ((sizeof(unsigned long) * count)/BITS_PER_LONG) + (sizeof(void*) * count), __GFP_HARDWALL | GFP_KERNEL|GFP_DMA );
    else
#endif
