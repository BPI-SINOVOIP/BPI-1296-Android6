...
static struct sg_table *dma_buf_te_map(struct dma_buf_attachment *attachment, enum dma_data_direction direction)
{
    ...
    if (!xen_initial_domain())
        sg = kmalloc(sizeof(struct sg_table), GFP_KERNEL|GFP_DMA);
    else
#endif
    ...
    if (!xen_initial_domain()) {
        if (alloc->contiguous)
            ret = sg_alloc_table(sg, 1, GFP_KERNEL|GFP_DMA);
        else
            ret = sg_alloc_table(sg, alloc->nr_pages, GFP_KERNEL|GFP_DMA);
    } else
#endif
	if (alloc->contiguous)
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        alloc = kzalloc(sizeof(struct dma_buf_te_alloc), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        alloc->pages = kzalloc(sizeof(struct page *) * alloc->nr_pages, GFP_KERNEL|GFP_DMA);
    else
#endif
	...
	if (contiguous) {
        ...
        if (!xen_initial_domain())
            alloc->contig_cpu_addr = dma_alloc_attrs(te_device.this_device,
                    ...
                    GFP_KERNEL|GFP_DMA | __GFP_ZERO, &attrs);
        else
#endif
		alloc->contig_cpu_addr = dma_alloc_attrs(te_device.this_device,
				alloc->nr_pages * PAGE_SIZE,
...
#ifdef CONFIG_RTK_XEN_SUPPORT
            if (!xen_initial_domain())
                alloc->pages[i] = alloc_page(GFP_KERNEL|GFP_DMA | __GFP_ZERO);
            else
#endif
