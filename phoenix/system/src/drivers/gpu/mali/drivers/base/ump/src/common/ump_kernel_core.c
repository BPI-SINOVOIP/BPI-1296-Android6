...
umpp_session *umpp_core_session_start(void)
{
    ...
    if (!xen_initial_domain())
        session = kzalloc(sizeof(*session), GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        alloc = kzalloc(sizeof(*alloc), GFP_KERNEL|GFP_DMA | __GFP_HARDWALL);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        alloc = kzalloc(sizeof(*alloc),__GFP_HARDWALL | GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        alloc->block_array = kzalloc(sizeof(ump_dd_physical_block_64) * num_blocks,__GFP_HARDWALL | GFP_KERNEL|GFP_DMA);
    else
#endif
...
#ifdef CONFIG_RTK_XEN_SUPPORT
    if (!xen_initial_domain())
        block_64_array = kzalloc(num_blocks * sizeof(*block_64_array), __GFP_HARDWALL | GFP_KERNEL|GFP_DMA);
    else
#endif
