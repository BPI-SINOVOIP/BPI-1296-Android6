					   ...
					   int size, const char *name)
{
	...
	if (size < sizeof(struct sync_timeline)) {
		printk(KERN_ERR "[%s %d] size(%d) < %d\n", __FUNCTION__, __LINE__, size, sizeof(struct sync_timeline));
		return NULL;
	}

	obj = kzalloc(size, GFP_KERNEL);
	if (obj == NULL) {
		printk(KERN_ERR "[%s %d] obj is NULL,size %d\n", __FUNCTION__, __LINE__, size);
		return NULL;
	}
...
struct sync_pt *sync_pt_create(struct sync_timeline *obj, int size)
{
	...
	if (size < sizeof(struct sync_pt)) {
		printk(KERN_ERR "[%s %d] size(%d) < %d\n", __FUNCTION__, __LINE__, size, sizeof(struct sync_pt));
		return NULL;
	}

	pt = kzalloc(size, GFP_KERNEL);
	if (pt == NULL) {
		printk(KERN_ERR "[%s %d] pt is NULL,size %d\n", __FUNCTION__, __LINE__, size);
		return NULL;
	}
...
static struct sync_fence *sync_fence_alloc(int size, const char *name)
{
	...
	if (fence == NULL) {
		printk(KERN_ERR "[%s %d] fence is NULL,size %d\n", __FUNCTION__, __LINE__, size);
		return NULL;
	}
	...
	if (IS_ERR(fence->file)) {
		printk(KERN_ERR "[%s %d] fence->file wrong(%d)\n", __FUNCTION__, __LINE__, IS_ERR(fence->file));
		goto err;
	}
...
struct sync_fence *sync_fence_create(const char *name, struct sync_pt *pt)
{
	...
	if (fence == NULL) {
		printk(KERN_ERR "[%s %d] fence is NULL\n", __FUNCTION__, __LINE__);
		return NULL;
	}
