...
void r8169soc_ocp_reg_write(struct rtl8169_private *tp, unsigned int reg_addr, unsigned int value){
	void __iomem *ioaddr = tp->mmio_addr;
	unsigned int wdata;

	// write value to reg
	wdata = (0x1 << 31) | (((reg_addr & 0xffff) >> 1) << 16) | (value & 0xffff);
	RTL_W32(OCPDR, wdata);
}

unsigned int r8169soc_ocp_reg_read(struct rtl8169_private *tp, unsigned int reg_addr){
	void __iomem *ioaddr = tp->mmio_addr;
	unsigned int wdata;
	unsigned int rdata;
