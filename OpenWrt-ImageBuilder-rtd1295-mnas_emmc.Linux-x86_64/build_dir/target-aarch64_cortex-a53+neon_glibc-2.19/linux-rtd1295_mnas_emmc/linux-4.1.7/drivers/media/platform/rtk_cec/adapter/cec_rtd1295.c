...
int rtk_cec_resume(rtk_cec* p_this)
{
	...
	write_reg32(p_this->reg_base+ CEC_CR0 , CEC_MODE(1) | LOGICAL_ADDR(0x4) |
						TIMER_DIV(cec_divider_table[p_this->speed_idx*3+2]) |
						PRE_DIV(cec_divider_table[p_this->speed_idx*3+1]) |
						UNREG_ACK_EN);
	write_reg32(p_this->reg_base+ CEC_RTCR0,  CEC_PAD_EN | CEC_PAD_EN_MODE |RETRY_NO(2));
...
static int ops_probe(cec_device* dev)
{
	...
	m_cec->clk_sys = clk_get(NULL, "clk_sys");
