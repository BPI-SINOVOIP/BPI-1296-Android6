...
void rtk_ahci_get_blkname(const char **name)
{
	*name = ahci_dev->blkname;
}
EXPORT_SYMBOL_GPL(rtk_ahci_get_blkname);
...
//	of_property_read_u32(dev->of_node, "hotplug-en", &ahci_dev->hotplug_en);
	...
	of_property_read_string(dev->of_node, "blkdev-name", &ahci_dev->blkname);
