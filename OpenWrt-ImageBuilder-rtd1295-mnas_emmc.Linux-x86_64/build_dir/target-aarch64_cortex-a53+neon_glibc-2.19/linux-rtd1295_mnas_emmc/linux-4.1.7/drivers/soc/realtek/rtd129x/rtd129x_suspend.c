...
struct _memory_verified_handle * memory_writeback_handle_create(struct _memory_verified_handle *phys_area)
{
    ...
    handle->memAddress = (char *)kmalloc(phys_area->memByte, GFP_KERNEL);
...
static int rtk_suspend_to_ram(void)
{
    ...
    mem_wbhandle = kmalloc(sizeof(phys_wb_area)/sizeof(struct _memory_verified_handle)*sizeof(memory_verified_handle_t*), GFP_KERNEL);   
    if (mem_wbhandle) {	
