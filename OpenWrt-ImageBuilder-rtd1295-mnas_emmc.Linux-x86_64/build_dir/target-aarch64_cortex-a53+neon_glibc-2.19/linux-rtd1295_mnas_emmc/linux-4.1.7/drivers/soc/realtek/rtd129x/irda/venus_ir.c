...
#define RAW_MODE 0xFF
//#define RAW_FIFOSIZE 1024
...
static struct venus_hw_param hw_param[] = {
	...
	{ {0x00000a8b}, {0x03138848}, 0x00001517 },		//RAW_CHARTER
};
...
struct rtk_ir_dev {
	...
	int rx_fifosize;
	int tx_fifosize;
	unsigned char *raw_txfifo;
	unsigned char *raw_rxfifo;
	...
	unsigned int tx_modulation;
	unsigned int tx_data_mode;
	unsigned int tx_inact_level;
...
int rtk_ir_init(void)
{
	...
	if(protocol==RAW_NEC || protocol==RAW_RC6 || protocol==RAW_DIRECTTV || protocol==RAW_COMCAST || protocol==RAW_CHARTER) {
	...
	} else if(protocol == RAW_MODE) {
		writel( (ir_dev->rx_samplerate*27 - 1), ir_dev->ir_reg+SATURN_ISO_IR_SF_OFF);
	...
	switch(protocol) {
		...
		case RAW_COMCAST:
			...
			break;
		case RAW_CHARTER:
			printk(KERN_ERR "[IRDA] RAW CHARTER\n");
			writel(0x7300, ir_dev->ir_reg+SATURN_ISO_IR_CR_OFF);
			break;
	}
	switch(ir_dev->irtx_protocol) {
		case NEC_TX:
			...
			break;
		case RAW_MODE:
			writel( ir_dev->tx_samplerate*27 - 1, ir_dev->ir_reg+SATURN_ISO_IRTX_TIM_OFF);

			reg = 1<<31 | ir_dev->tx_inact_level<<10 | ir_dev->tx_inact_level << 4;
			if (ir_dev->tx_modulation) {
				writel(0x13A00 | (6750 / ir_dev->tx_modulation) - 1,
					ir_dev->ir_reg+SATURN_ISO_IRTX_PWM_SETTING_OFF);
				reg = reg | 1<<9;
			}
			writel(reg, ir_dev->ir_reg+SATURN_ISO_IRTX_CFG_OFF);
			// Data Threshold=16, Data finish Interrupt Enable, Data Empty Interrupt Disable, Data Request Interrupt Disable
...
static int repeat_key_handle(unsigned int regValue, unsigned int irrp2Value, int dataRepeat)
{
	...
	if((RPRepeatIsHandling== true) && ((jiffies_to_msecs(jiffies)- ir_dev->lastRecvMs) < debounce)) {
		...
		} else if(ir_dev->ir_protocol == RAW_DIRECTTV || ir_dev->ir_protocol == RAW_COMCAST || ir_dev->ir_protocol == RAW_CHARTER) {
//			printk(KERN_ERR "msg_code = 0x%04x\n", regValue);
			if(ir_dev->p_rtk_key_table == &rtk_mk5_tv_key_table) {
				keycode = regValue;
			}
			else {
				printk(KERN_ERR "have key table\n");
...
static int charter_length_decode(unsigned int len_h, unsigned int len_l)
{
	unsigned int len_total;
...
static int raw_charter_decoder(int *dataRepeat)
{
	unsigned int srValue = readl(ir_dev->ir_reg+SATURN_ISO_IR_SR_OFF);
...
decode_start:
		if(fifostate<2) {
			if(fifostate == 1) {
				if(length_high==0 || length_low==0 || length_lb==0 || length_hb==0) {
					...
					if((length_hb>=43 && length_hb<=47)) {
						if(length_lb>=10 && length_lb <=13) {
							symbol = lastsymbol;
							symbol_cnt = 16;
						} else if(length_lb>=21 && length_lb<=24)
							symbol_cnt = symbol = 0;
						fifostate = 2;
					} else
						fifostate = 0;
		...
		else if(fifostate>=2 && symbol_cnt<=16) {
			if(fifostate==3) {
				fifostate = 2;
				if(length_high==0 || length_low==0 || length_hb==0 || length_lb==0) {
					...
					code = charter_length_decode(length_hb, length_lb);
					if(code < 0)
						fifostate = symbol_cnt = symbol = 0;
					else
						symbol = symbol | code << (15-symbol_cnt++);
				}
				length_hb = length_lb = 0;
			}
			if(symbol_cnt < 16) {
				code = charter_length_decode(length_high, length_low);
				if(code < 0)
					fifostate = symbol_cnt = symbol = 0;
				else
					symbol = symbol | code << (15-symbol_cnt++);
			...
			if(symbol_cnt==16) {
				lastsymbol = symbol & 0xffff;
				valid_code[codenum++] = symbol & 0xffff;
				printk(KERN_ERR "Decode success 0x%x\n", symbol);
				fifostate = symbol = symbol_cnt = 0;
			}
		}
	}
	if(fifostate==2 || (length_high>0 && length_low==0) || length_high>40 ) {
...
//		printk(KERN_ERR "len = %d\n", len);
	if(len < 512 && start_tx==0) {
		...
		if(!(srValue & 0x1)) {
			...
			srValue |= 0x1;
			writel(srValue, ir_dev->ir_reg+SATURN_ISO_IRTX_CFG_OFF);  //start Tx
		}
		start_tx=1;
	}
...
static irqreturn_t rtk_ir_isr(int irq, void *dev_id)
{
	...
	if(ir_dev->ir_protocol == RAW_NEC) {
		...
		if(regVal>=0)
			received = repeat_key_handle(regVal, irrp2Value, dataRepeat);
	} else if(ir_dev->ir_protocol == RAW_CHARTER) {
		regVal = raw_charter_decoder(&dataRepeat);
		if(regVal>=0)
			received = repeat_key_handle(regVal, irrp2Value, dataRepeat);
	} else if(ir_dev->ir_protocol == RAW_MODE) {
		get_raw_data();
	} else {
...
CONTINUE_IRTX:
	...
	if(ir_dev->irtx_protocol == RAW_MODE) {
		if(regValue & 0x2) {
			...
			if(txlen > (FIFO_DEPTH-1))
				txlen = FIFO_DEPTH - 1;
			if(txlen==0) {
				printk(KERN_INFO "[IRTX] no data in fifo\n");
				writel(0xf04, ir_dev->ir_reg+SATURN_ISO_IRTX_INT_EN_OFF);
			} else {
		...
		if (regValue & 0x4) {
			printk(KERN_INFO "[IRTX] stop tx 0x%x\n", regValue);
			rtk_irtx_enable(0);
...
int rtk119x_ir_remove(struct platform_device * pdev)
{
	...
	if(ir_dev->ir_protocol == RAW_MODE)
		devm_kfree(dev, ir_dev->raw_rxfifo);
	if(ir_dev->irtx_protocol == RAW_MODE)
		devm_kfree(dev, ir_dev->raw_txfifo);
...
int prepare_txfifo_from_data(unsigned int *time, size_t count, unsigned char *fifo)
{
	...
	cnt = 0;
	for (i=0; i<(count / 4); i++)
		cnt += time[i] / ir_dev->tx_samplerate;
	if (cnt % 32)
		cnt = ((cnt / 32) + 1) * 32;
	cnt = cnt / 8;
	...
	fifocnt = remain = 0;
	for (i=0; i<(count / 4); i++) {
		realcnt = time[i] / ir_dev->tx_samplerate;
		if (remain) {
			for (j=remain; j<8; j++) {
				if (i % 2)
					fifo[fifocnt] &= ~(0x1 << j);
...
ssize_t rtk_ir_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if(ir_dev->irtx_protocol == RAW_MODE) {
		...
		if (ir_dev->tx_data_mode) {
			if (count > 2048) {
				RTK_debug(KERN_ALERT "write error: too many data!\n");
				return -EFAULT;
			}
			if(copy_from_user(time, buf, count)) {
				RTK_debug(KERN_ALERT "venus_ir_write : copy from user error!\n");
				return -EFAULT;
			}
			ret = prepare_txfifo_from_data(time, count, ir_dev->raw_txfifo);
			if (ret <= 0 || ret > write_Count)
				return -EFAULT;
			else
				write_Count = ret;
		} else {
			...
			if(copy_from_user(ir_dev->raw_txfifo, buf, write_Count)) {
				RTK_debug(KERN_ALERT "venus_ir_write : copy from user error!\n");
				return -EFAULT;
			}
		}
		...
		if(!(reg & 0x1)) {
			if(write_Count <= FIFO_DEPTH*4) {
				for(i=0; i<(write_Count/4); i++)
					writel( *((int *)(ir_dev->raw_txfifo)+i), ir_dev->ir_reg+SATURN_ISO_IRTX_FIFO_OFF);
				writel(reg | 0x1, ir_dev->ir_reg+SATURN_ISO_IRTX_CFG_OFF);  //start Tx
			} else {
				for(i=0; i<FIFO_DEPTH-1; i++)
					writel( *((int *)(ir_dev->raw_txfifo)+i), ir_dev->ir_reg+SATURN_ISO_IRTX_FIFO_OFF);
				writel(reg | 0x1, ir_dev->ir_reg+SATURN_ISO_IRTX_CFG_OFF);  //start Tx
				fiforet = kfifo_in(&ir_cdev->ir_txfifo, ir_dev->raw_txfifo + (FIFO_DEPTH-1)*4, write_Count - (FIFO_DEPTH-1)*4);
			}
		} else {
			kfifo_in(&ir_cdev->ir_txfifo, ir_dev->raw_txfifo, write_Count);
		}
		return write_Count;
	}
...
restart:
	...
	if(ir_dev->ir_protocol == RAW_MODE) {
		len = kfifo_out(&ir_cdev->ir_rxfifo, ir_dev->raw_rxfifo, readCount);
		readCount = len;
		if(copy_to_user(buf, ir_dev->raw_rxfifo, len)) {
...
long rtk_ir_compat_ioctl(struct file* file,unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case VENUS_IRRX_RAW_STOP:
			return file->f_op->unlocked_ioctl(file, cmd,(unsigned long)compat_ptr(arg));
		case VENUS_IRTX_IOC_SET_MODULATION:
			return file->f_op->unlocked_ioctl(file, cmd,(unsigned long)compat_ptr(arg));
		default:
			return -EFAULT;
	}
...
INIT:
	if (ir_dev->ir_protocol == RAW_MODE) {
		if(of_property_read_u32(dev->of_node, "Realtek,irrx-fifosize", &ir_dev->rx_fifosize))
			ir_dev->rx_fifosize = 1024;
		if(of_property_read_u32(dev->of_node, "Realtek,irrx-samplerate", &ir_dev->rx_samplerate))
			ir_dev->rx_samplerate = 200;
		ir_dev->raw_rxfifo = devm_kzalloc(dev, ir_dev->rx_fifosize, GFP_KERNEL);
		if(!ir_dev->raw_rxfifo) {
			dev_err(dev, "[%s]: raw_rxfifo alloc failed\n", __func__);
			return -ENOMEM;
		}
	} else {
		ir_dev->rx_fifosize = FIFO_DEPTH*sizeof(unsigned int);
	}
	if (ir_dev->irtx_protocol == RAW_MODE) {
		if(of_property_read_u32(dev->of_node, "Realtek,irtx-fifosize", &ir_dev->tx_fifosize))
			ir_dev->tx_fifosize = 1024;
		if(of_property_read_u32(dev->of_node, "Realtek,irtx-samplerate", &ir_dev->tx_samplerate))
			ir_dev->tx_samplerate = 20;
		if (of_property_read_u32(dev->of_node, "Realtek,irtx-modulation", &ir_dev->tx_modulation))
			ir_dev->tx_modulation = 0;
		if (of_property_read_u32(dev->of_node, "Realtek,irtx-data-mode", &ir_dev->tx_data_mode))
			ir_dev->tx_data_mode = 0;
		if (of_property_read_u32(dev->of_node, "Realtek,irtx-inact_level", &ir_dev->tx_inact_level))
			ir_dev->tx_inact_level = 1;
		ir_dev->raw_txfifo = devm_kzalloc(dev, ir_dev->tx_fifosize, GFP_KERNEL);
		if(!ir_dev->raw_txfifo) {
			dev_err(dev, "[%s]: raw_txfifo alloc failed\n", __func__);
			return -ENOMEM;
		}
	} else {
		ir_dev->tx_fifosize = FIFO_DEPTH*sizeof(unsigned int);
	}
	...
	if(kfifo_alloc(&ir_cdev->ir_rxfifo, ir_dev->rx_fifosize, GFP_KERNEL)) {
		dev_err(dev, "[%s]: kfifo_alloc failed\n", __func__);
		return -ENOMEM;
	}
	if(kfifo_alloc(&ir_cdev->ir_txfifo, ir_dev->tx_fifosize, GFP_KERNEL)) {
		dev_err(dev, "[%s]: kfifo_alloc failed\n", __func__);
		return -ENOMEM;
	}
