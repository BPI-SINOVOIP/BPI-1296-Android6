...
define DownloadMethod/git
	$(call wrap_mirror, \
		...
		[ \! -d $(SUBDIR) ] && \
		if [ -z $(BRANCH) ]; then \
		git clone $(if $(shell echo $(CONFIG_GIT_REPO_USERNAME)),$(if $(filter realtek, $(subst ., ,$(URL))),$(subst ssh://,ssh://$(CONFIG_GIT_REPO_USERNAME)@,$(URL)),$(URL)),$(URL)) $(SUBDIR); \
		else \
		git clone --branch $(BRANCH)  $(if $(shell echo $(CONFIG_GIT_REPO_USERNAME)),$(if $(filter realtek, $(subst ., ,$(URL))),$(subst ssh://,ssh://$(CONFIG_GIT_REPO_USERNAME)@,$(URL)),$(URL)),$(URL)) $(SUBDIR); \
		fi && \
