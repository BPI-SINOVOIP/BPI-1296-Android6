...
drv_mac80211_init_device_config() {
	...
	config_add_boolean noscan ht_coex txbf
...
mac80211_prepare_rtkmesh() {
	...
	json_select config
	json_get_vars mode ssid encryption key
...
mac80211_prepare_vif() {
	...
	json_get_vars ifname mode ssid wds powersave macaddr rtk_wds tdls_prohibited tdls_cs_prohibited ieee80211k
	...
	[ -d /proc/$ifname ] && [ -z $( echo $ifname | grep [-.]) ] && {
		if [ "$txbf" -gt 0 ]; then
			iwpriv $ifname set_mib txbf=1
		else
			iwpriv $ifname set_mib txbf=0
		fi
	}
...
drv_mac80211_setup() {
	...
	json_get_vars \
		...
		rxantenna txantenna \
		frag rts beacon_int htmode txbf
