...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
    ...
    private boolean ForceStop() {
        if (SystemProperties.get("rtk.mediascanner.forcestopscan").equals(
                "true") == true)
            return true;
        else
            return false;
        ...
        try {
            if (prescanFiles) {
                ...
                while (true) {
                    ...
                    c = mMediaProvider.query(mPackageName, limitUri, projectionIn,
                ...
                if (bRemovedNonExistingDB) {
                    for (long id : nonExistStorageIDs) {
                        ...
                        mMediaProvider.call(mPackageName, "remove", db, null);
                    }
                }
            }
        }finally {
    ...
    static class MediaBulkDeleter {
        ...
        public void deleteAll() throws RemoteException {
        	mProvider.delete(mPackageName, mBaseUri, "", null);
        }
    ...
    //[RTK Begin]
    private void postscan(String[] directories, boolean[] needScanned) throws RemoteException {
    //[RTK End]
            ...
            for (int i = 0; i < directories.length; i++) {
                ...
                if (IsExternalStorageVolumeRoot(directories[i]) && !bRetPrimary) {
                    if (false == needScanned[i])
                        mMediaProvider.call(mPackageName, "merge", directories[i], null);
                    else
                    {
                    	mMediaProvider.call(mPackageName, "merge", directories[i], null);
                    	mMediaProvider.call(mPackageName, "copy", directories[i], null);
                    }
                }
            }
        }
    ...
    private boolean needScanTheStorage(String filePath) {
        ...
        try {
            ...
            c = mMediaProvider.query(mPackageName, mFilesUri, FILES_FREESPACE_PROJECTION,
    ...
    public void scanDirectories(String[] directories, String volumeName) {
        SystemProperties.set("rtk.mediascanner.scanning","true");
        if(ForceStop()==true){
            SystemProperties.set("rtk.mediascanner.scanning","false");
            return;
        }
...
//                mMediaInserter = new MediaInserter(mMediaProvider, mPackageName, 10);
...
//
    ...
    private void processPlayLists() throws RemoteException {
        ...
        try {
            ...
            fileList = mMediaProvider.query(mPackageName, Uri.parse("content://media/external/file"), FILES_PRESCAN_PROJECTION,
    ...
    private void reprocessPlayLists() throws RemoteException {
        ...
        try {
            ...
            cursor_playlists = mMediaProvider.query(mPackageName,
        ...
        mMediaProvider.call(mPackageName, "delete_all_audio_playlists_map",
                null, null);
        ...
        try {
            ...
            // column,
            // but restrict the query to just audio files
            fileList = mMediaProvider.query(mPackageName,
