...
#ifdef MS12_FLOW_EN
    virtual status_t setMS12FlowConfig(uint32_t cfg_addr);
#endif
...
private:
...
#if 1
    int64_t mInitialReadTimeUs;       /* Initial Read Timestamp */
#endif
    int64_t mLastOutputTimeUs;      /* Last OutputBuffer PTS */
    int64_t mLastInputTimeUs;       /* Last InputBuffer  PTS */
    int64_t mPauseTargetTimeUs;     /* If mPauseTargetTimeUs is not equal to -1,
                                     * it will turn off the DrainBufferInterval check.
                                     */
    ...
    status_t setRTK_ALACFormat(const sp<MetaData>& meta );
#endif
    ...
    //video
    status_t setWMVFormat(OMX_VIDEO_WMVFORMATTYPE wmvFormat);
};
