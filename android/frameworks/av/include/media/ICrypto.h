...
struct ICrypto : public IInterface {
    ...
    virtual ssize_t decrypt(
            ...
            const CryptoPlugin::SubSample *subSamples, size_t numSubSamples,
            void *dstPtr,
            AString *errorDetailMsg) {return -2;} ;

    virtual uint32_t checkPlugin(uint32_t maginNumber) { return 0x1234;};

    //NOTICE: assume bn & bp are in the same process, so that srcPtr & dstPtr are valid
    virtual ssize_t encrypt(bool secure, const void *srcPtr, void *dstPtr, size_t size) = 0;
