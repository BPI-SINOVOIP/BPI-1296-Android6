...
-            device, state, device_address, device_name);
    ...
    bool usb_out = false;
    if (property_get("persist.rtk.usbout.audio", propvalue, "1") && atoi(propvalue) == 1) {
        usb_out = true;
    } else {
    ...
    if (device == AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET && usb_out == false && state == AUDIO_POLICY_DEVICE_STATE_AVAILABLE) {
        ALOGD("no set usb audio");
        return BAD_VALUE;
    }
#endif
...
//rtk begin
            ...
            else if((device & AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET) || (device & AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET)|| (device & AUDIO_DEVICE_OUT_USB_DEVICE)) {
    ...
    if (profile != 0) {
        ...
        for (size_t i = 0; i < mOutputs.size(); i++) {
            ...
            if (!desc->isDuplicated() && (profile == desc->mProfile)) {
                ...
                /* rtk modify: DO NOT reuse direct output for IEC61937 */
				if (format == AUDIO_FORMAT_IEC61937) {	
	              ALOGW("getOutput() DOES NOT reuse direct output %d with the same parameters", mOutputs.keyAt(i));
                }
                else if ((samplingRate == outputDesc->mSamplingRate) &&
...
//rtk add
            if (device != AUDIO_DEVICE_OUT_DEFAULT || curDevice != AUDIO_DEVICE_OUT_SPEAKER || index != 11) {
//rtk end
...
status_t AudioPolicyManager::setHDMIRXvolume(int volume)
{
    ...
    if(mHDMIRXAudioPIN != 0){ /* have HDMI Rx */
...
bool AudioPolicyManager::isOffloadSupported(const audio_offload_info_t& offloadInfo)
{
    ...
    if (property_get("rtk.hdmi.output", propValue, "1")) {
        if (atoi(propValue) != 0) {
            ALOGD("offload disabled by hdmi setting vaule %s", propValue );
            return false;
        }
    }
...
//rtk begin
    isUSBConnected(false), isBTConnected(false),
    isHDMIMuted(false), isHDMIRXConnected(false),
    mHDMIRXvolume(MAX_RTK_VOLUME),
    mAudioRedirect(false),
    mStreamVolumeIndex(11),
    mRTKAudioActive(false),
    mHDMIRXAudioAOID(0), mHDMIRXAudioAIID(0), mHDMIRXAudioPIN(0), mBootComplete(false)
//rtk end
{
...
//rtk begin
    ...
    if (stream == AUDIO_STREAM_MUSIC) {
        ...
        if(!isHDMIMuted && device == AUDIO_DEVICE_OUT_SPEAKER && mBootComplete)
        {
            ...
            memset(propvalue, 0x0, sizeof(propvalue));
            property_get("persist.rtk.hdmi.audio", propvalue, "on");
