...
//[ RTK begin ]
    ...
    virtual void setSideBand(node_id node, bool enableSideBand) {
        Parcel data, reply;
        data.writeInterfaceToken(IOMX::getInterfaceDescriptor());
        data.writeInt32((int32_t)node);
        data.writeInt32(enableSideBand);
        remote()->transact(SET_SIDEBAND, data, &reply);
    }
    ...
    switch (code) {
        ...
        case ALLOC_BUFFER:
        {
            ...
            if (!isSecure(node) || port_index != 0 /* kPortIndexInput */) {
                ALOGV("\033[0;35m @@@@@@@@ In %s:%d isSideBand %d @@@@@@\033[m", __FUNCTION__, __LINE__, isSideBand(node));
                if (!isSideBand(node))
                {
                    ALOGE("b/24310423");
        ...
        /* Reduce omx input-port memcpy by SideBand */
        case SET_SIDEBAND:
        {
            CHECK_OMX_INTERFACE(IOMX, data, reply);
            ...
            ALOGV("\033[0;32m $$$$$ In %s:%d call setSideband , option:%d ##### \033[m", __FUNCTION__, __LINE__, option);
            setSideBand(node, option);
            return OK;
        }
        //[ RTK end ]
