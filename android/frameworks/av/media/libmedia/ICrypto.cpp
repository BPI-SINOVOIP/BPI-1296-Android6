...
struct BpCrypto : public BpInterface<ICrypto> {
    ...
    virtual ssize_t decrypt(
        ...
        if (drmMessageLen > 0)
        {
            ...
            if (drmMessageData[TRANSFER_SRC_SHAREDFD] > 0)
            {
                status = data.writeDupFileDescriptor(drmMessageData[TRANSFER_SRC_SHAREDFD]);
                if (status)
                    ALOGE("\033[0;35m ---- In %s:%d status:%d strerror:%s failed to TRANSFER_SRC_SHAREDFD------\033[m", __FUNCTION__, __LINE__, status, strerror(status));
            }
            else
            {
                ALOGE("\033[0;35m ---- In %s:%d recevied wrong TRANSFER_SRC_SHAREDFD:%d ------\033[m", __FUNCTION__, __LINE__, drmMessageData[TRANSFER_SRC_SHAREDFD]);
                return ERROR_DRM_CANNOT_HANDLE;
            }
            drmMessageLen--;

            if (drmMessageData[TRANSFER_DST_SHAREDFD] > 0)
            {
                status = data.writeDupFileDescriptor(drmMessageData[TRANSFER_DST_SHAREDFD]);
                if (status)
                    ALOGE("\033[0;35m ---- In %s:%d status:%d strerror:%s failed to TRANSFER_DST_SHAREDFD------\033[m", __FUNCTION__, __LINE__, status, strerror(status));
            }
            else
            {
                ALOGE("\033[0;35m ---- In %s:%d recevied wrong TRANSFER_DST_SHAREDFD:%d ------\033[m", __FUNCTION__, __LINE__, drmMessageData[TRANSFER_DST_SHAREDFD]);
                return ERROR_DRM_CANNOT_HANDLE;
            }
...
status_t BnCrypto::onTransact(
    ...
    switch (code) {
        ...
        case DECRYPT_RTK:
        {
            ...
            /* Src shared Fd */
            drmMessageData[TRANSFER_SRC_SHAREDFD] = dup(data.readFileDescriptor()); // close in libmediaplayerservice/Crypto.cpp
            /* Dst shared Fd */
            drmMessageData[TRANSFER_DST_SHAREDFD] = dup(data.readFileDescriptor()); // close in libmediaplayerservice/Crypto.cpp
