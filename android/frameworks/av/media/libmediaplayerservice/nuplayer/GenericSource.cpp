...
#ifdef RTK_PLATFORM
static int64_t kLowWaterMarkUs = 100000ll;  // 0.2secs
#else
...
#ifdef RTK_PLATFORM
ssize_t kHighWaterMarkBytes = 200000;
static const ssize_t kIniHighWaterMarkBytes = 200000;
#else
...
#ifdef RTK_PLATFORM
    if(mInStartPlayBackFstChkBuffer == false)
    {
        msg->post(3000000ll);
...
#ifdef RTK_PLATFORM
void NuPlayer::GenericSource::onRTKCustomerHighWaterMBsChk()
{
    ...
    if (property_get(RTK_PROPERTY_MEDIA_BUFFER, value, NULL))
    {
        ...
        if(ctrl > 0)
        {
            ALOGV("[%s][%d] set RTK_PROPERTY_MEDIA_BUFFER :: %f MBytes\n",__FUNCTION__,__LINE__,ctrl);
            kHighWaterMarkBytes = (ssize_t)(ctrl*1000000);
            //NuCachedSource2::kDefaultHighWaterThreshold is 40MBytes
            if(kHighWaterMarkBytes > (40*1024*1024))
            {
                kHighWaterMarkBytes = (39*1024*1024);
                ALOGW("[%s][%d] kHighWaterMarkBytes set wrong %f MBytes,reset 39 MBytes\n",__FUNCTION__,__LINE__,ctrl);
            }
        }
    }
    else
        kHighWaterMarkBytes = kIniHighWaterMarkBytes;
}
#endif
...
void NuPlayer::GenericSource::onPollBuffering() {
    ...
    if (finalStatus != OK) {
        ...
        if (cachedDurationUs < kLowWaterMarkUs) {
            ...
            if(mPrevBufferCacheSize >kLowWaterMarkBytes ||
                    ((int64_t)cachedDataRemaining > mPrevBufferCacheSize && (((int64_t)cachedDataRemaining - mPrevBufferCacheSize) > kLowWaterMarkBytes)) )
            {
                if(mBuffering == false)
                    ensureCacheIsFetching();
            }else{
        ...
        mPrevBufferCacheSize = cachedDataRemaining;
...
void NuPlayer::GenericSource::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
      ...
      case kWhatResume:
      {
          ...
          mPrevBufferCacheSize = kIniHighWaterMarkBytes;
          restartPollBuffering();
          break;
      }
