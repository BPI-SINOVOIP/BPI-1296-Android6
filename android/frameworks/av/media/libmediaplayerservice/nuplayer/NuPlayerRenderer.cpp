...
NuPlayer::Renderer::Renderer(
    ...
    mPreVideoLateByUs = 0;
    mAudioEOS = false;
#ifdef ENBLE_SLOW_DELAY
//    char value[PROPERTY_VALUE_MAX];
    mEnbSlowDelayFlg =false;
//    if (property_get("persist.rtk.searchI", value, NULL) && (!strcmp("1", value)|| !strcmp("true", value)))
    {
        ALOGI("[%s][%d] ENB Slow Delay in seek flow.\n",__FUNCTION__,__LINE__);
        mEnbSlowDelayFlg = true;
    }
    mHLSSeekDoneFlg = false;
    mFirstVideoRenderFrame = false;
    mFirstAudioRenderFrame = false;
    gettimeofday(&e_systime, NULL);
    gettimeofday(&s_systime, NULL);
    mVideoFirstMediaUs = -1;
#endif
    ...
    if (hasEOS) {
        ...
        if (!offloadingAudio()) {
            ...
            ALOGV("fillAudioBuffer: notifyEOS "
                    ...
                    (unsigned long long)mNumFramesWritten, entry->mFinalResult, (long long)postEOSDelayUs);
...
#endif
#ifdef ENBLE_SLOW_DELAY
    if(mHasVideo == true && mEnbSlowDelayFlg == true && mFirstVideoRenderFrame == true && mFirstAudioRenderFrame == false &&
            !mAudioQueue.empty() && mVideoFirstMediaUs >= 0)
    {
        while (!mAudioQueue.empty())
        {
            ...
            if (entry->mOffset == 0 && entry->mBuffer->size() > 0)
            {
                int64_t mediaTimeUs;
                CHECK(entry->mBuffer->meta()->findInt64("timeUs", &mediaTimeUs));
                if(mediaTimeUs >= (mVideoFirstMediaUs + 300000ll))
                {
                    mFirstAudioRenderFrame = true;
                    ALOGI("[%s][%d] mFirstAudioRenderFrame set true mediaTimeUs::%f %f\n",__FUNCTION__,__LINE__,
                            (double)mediaTimeUs/1E6,(double)mVideoFirstMediaUs/1E6);
                    break;
                }
                entry->mNotifyConsumed->post();
                entry = NULL;
        ...
        ALOGD("reschedule:%d  empty:%d  mPaused:%d  mNumFramesWritten:%llu",
                reschedule, mAudioQueue.empty(), mPaused,  (unsigned long long)mNumFramesWritten);
        return reschedule;
    }
...
#endif
#ifdef ENBLE_SLOW_DELAY
    while (!mAudioQueue.empty() && mFirstAudioRenderFrame == true)
#else
    while (!mAudioQueue.empty())
#endif
    {
...
int64_t NuPlayer::Renderer::getRealTimeUs(int64_t mediaTimeUs, int64_t nowUs) {
    ...
    if (mMediaClock->getRealTimeFor(mediaTimeUs, &realUs) != OK) {
        ...
        ALOGW("[%s][%d] fail to getRealTimeFor use nowUs:%f\n",__FUNCTION__,__LINE__,codec_show_pts(nowUs));
        return nowUs;
    }
    return realUs;
}
...
void NuPlayer::Renderer::onNewAudioMediaTime(int64_t mediaTimeUs) {
    ...
    int64_t nowMediaUs = mediaTimeUs - mgetPendingAudioPlayoutDurationUs;
#ifdef RTK_PLATFORM
    if(mAnchorTimeMediaUs != -1 && Diff(mAnchorTimeMediaUs,mediaTimeUs) > 100000000ll)
    {
        ALOGW("[%s][%d] ERROR Status %f %f %f %f %f %lld\n",__FUNCTION__,__LINE__,
                codec_show_pts(mAnchorTimeMediaUs),codec_show_pts(nowMediaUs),
                codec_show_pts(nowUs),codec_show_pts(mediaTimeUs),
                codec_show_pts(getPendingAudioPlayoutDurationUs(nowUs)), (long long)mNumFramesWritten);
#ifdef RTK_ENB_QUEUE_DEBUG_LOG
        POPQueueDBGInfo();
#endif
    }
    ...
    QueueDBGInfo(true, false, false, nowMediaUs, nowUs, mgetPendingAudioPlayoutDurationUs);
#endif
#endif
...
void NuPlayer::Renderer::postDrainVideoQueue() {
    ...
    delayUs = realTimeUs - nowUs;
#ifdef ENBLE_SLOW_DELAY
    if(mEnbSlowDelayFlg == true)
    {
        gettimeofday(&e_systime, NULL);
        unsigned int duration = (e_systime.tv_sec - s_systime.tv_sec)*1000 + (e_systime.tv_usec - s_systime.tv_usec)/1000;
        if(duration < 3000)
        {
            delayUs  = delayUs/2;
        }
    ...
    if (!mPaused) {
        ...
        if(mEnbSlowDelayFlg == true)
        {
            ...
            if(duration < 10000)
            {
                ...
                if(!tooLate)
                {
                    setVideoLateByUs((nowUs - realTimeUs)/2);
                    realTimeUs = realTimeUs +(nowUs - realTimeUs)/2;
                }
        ...
        if (!mHasAudio)
            tooLate = false;
        else if(tooLate && mPreVideoLateByUs < realTimeUs
                && (realTimeUs - mPreVideoLateByUs) < 33000
                && (mVideoLateByUs < 500000))
        {
            tooLate = false;
            ALOGV("[%s][%d] Set tooLate false diff(realTimeUs, mPreVideoLateByUs):%f < 33ms \n",
...
#ifdef ENBLE_SLOW_DELAY
    if(mEnbSlowDelayFlg == true && mHLSSeekDoneFlg == false)
    {
        //drop 0.3s data
        ALOGV("[%s][%d] diff :: %f  mediaTimeUs %f %f\n",__FUNCTION__,__LINE__,codec_show_pts(Diff(mVideoFirstMediaUs,mediaTimeUs)),
                codec_show_pts(mediaTimeUs),codec_show_pts(mVideoFirstMediaUs));
        if(mVideoFirstMediaUs == -1)
            mVideoFirstMediaUs = mediaTimeUs;
        if(Diff(mVideoFirstMediaUs,mediaTimeUs) < 300000ll)
        {
            tooLate = true;
        }
        else
        {
            mHLSSeekDoneFlg = true;
        }
        if(mFirstVideoRenderFrame == false)
        {
            mFirstVideoRenderFrame = true;
        }
    }
#endif
...
void NuPlayer::Renderer::setStartPBDropFrame(int32_t dIsEnbDropDrame)
{
    if(dIsEnbDropDrame == 1)
    {
        ALOGD("[%s][%d] HLS EnbDropFrame\n",__FUNCTION__,__LINE__);
        mHLSSeekDoneFlg = false;
        mFirstVideoRenderFrame = false;
        mFirstAudioRenderFrame = false;
        gettimeofday(&e_systime, NULL);
