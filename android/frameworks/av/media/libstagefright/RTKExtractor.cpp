...
{
    ALOGD("%s:%s:%d rtkVendorLibHandle=%p\n", __FILE__, __func__, __LINE__, rtkVendorLibHandle);
    rtkVendorLibHandle = dlopen("libRTKExtractor.so", RTLD_NOW);
    
    if (rtkVendorLibHandle == NULL) {
        ALOGD("%s:%s:%d af dlopen rtkVendorLibHandle=%p  dlerror(): %s\n", __FILE__, __func__, __LINE__, rtkVendorLibHandle, dlerror());
    }
...
sp<MediaSource> RTKExtractor::getTrack(size_t index) {
