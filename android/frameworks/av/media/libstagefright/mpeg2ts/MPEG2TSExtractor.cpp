...
void MPEG2TSExtractor::ThreadEntry()
{
    ...
    while (!mStopThread)
    {
        ...
        if(duration < bufferingTimeUs || mSeeking == true)
        {
            ...
            if(mStopAutoFeed)
            {
                ALOGV("There is no source to read, wait if we got repeat case");
                usleep(100000);
                continue;
            }
            status = feedMore();
...
#else
    while (forceStop == false && feedMore() == OK) {
#endif
        if (!haveVideo) {
...
status_t MPEG2TSExtractor::feedMore() {
    ...
    status_t err;
#ifdef RTK_PLATFORM
    for(size_t k=0;k<(n/mRealPacketSize);k++)
    {
