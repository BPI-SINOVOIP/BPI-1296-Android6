...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitPCMAudio() {
    ...
    if(bitPerSample == 24)
    {
        int8_t *ptr = (int8_t *)accessUnit->data();
        for (size_t i = 0; i < payloadSize / (sizeof(int8_t)*3); ++i) {
            int8_t first = ptr[3*i+2];
            int8_t third = ptr[3*i];
            ptr[3*i] = first;
            ptr[3*i+2] = third;
        }
    }
    ...
    {
        int32_t *ptr = (int32_t *)accessUnit->data();
        for (size_t i = 0; i < payloadSize / sizeof(int32_t); ++i) {
            ptr[i] = ntohl(ptr[i]);
        }
    }
    else
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitAAC() {
    ...
    if(timeUs < mSkipFrameAfterSeekTimestamp||mSeeking){
        ALOGV("Audio timeUs %" PRId64 " mSkipFrameAfterSeekTimestamp %" PRId64 " drop it ", timeUs, mSkipFrameAfterSeekTimestamp);
        return NULL;
    }
    if(mSkipFrameAfterSeekTimestamp != -1)
