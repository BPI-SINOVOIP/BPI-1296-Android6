...
MPEG2PSExtractor::MPEG2PSExtractor(const sp<DataSource> &source)
    ...
    for (size_t i = 0; i < 2000; ++i) {
        ...
        if(i > 1000)
        {
            bool allFormatDone = true;
            for (size_t j = mTracks.size(); j > 0;) {
                j--;
                if (mTracks.valueAt(j)->getFormat() == NULL) {
                    allFormatDone = false;
                    break;
                }
            }

            if(allFormatDone == true)
                break;
        }
    }
...
status_t MPEG2PSExtractor::Track::read(
    ...
    if (options && options->getSeekTo(&seekTimeUs, &seekMode)) {
        ...
        if(((mExtractor->mLastSeekTimeUs - seekTimeUs < 3000000) && (mExtractor->mLastSeekTimeUs - seekTimeUs > -3000000)))
