...
bool ATSParser::Program::needAVSync() {
    for (size_t i = 0; i < mStreams.size(); ++i) {
        if(mStreams.editValueAt(i)->isVideo()&&mStreams.editValueAt(i)->needAVSync()){
            return true;
        }
    }
    return false;
...
ATSParser::Stream::Stream(
    ...
    switch (mStreamType) {
        ...
        case STREAMTYPE_H265_CASE2:
            if(mCreateByMPEG2TSExt == SOURCE_FROM_M3U8)
            {
                mQueue = new ElementaryStreamQueue(
...
#ifdef RTK_PLATFORM
bool ATSParser::Stream::needAVSync() const {
    if (mStreamType == STREAMTYPE_H264
            || mStreamType == STREAMTYPE_H265
            || mStreamType == STREAMTYPE_H265_CASE2
            || mStreamType == STREAMTYPE_H265_CASE3){
        return true;
    }
    return false;
}
...
void ATSParser::Stream::onPayloadData(
    ...
    if(((timeUs>mBaseTime)?(timeUs-mBaseTime):(mBaseTime-timeUs)) < 3000000)
    {
...
#endif
#ifdef RTK_PLATFORM
            if(needAVSync()&&mIsVideoSizeSuitable){
                sp<MetaData> meta = mQueue->getFormat();
