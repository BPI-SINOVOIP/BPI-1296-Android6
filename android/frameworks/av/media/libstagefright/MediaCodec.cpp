...
#define RTK_DRM_PLUGIN
#ifdef RTK_DRM_PLUGIN // For Realtek DRM Plugin
#include <ion/ion.h>
#include <sys/mman.h>
#include <cutils/properties.h>
#define CHECK_SRC_PHYADDR(x)   (x & 0x1)
#define CHECK_DST_PHYADDR(x)   (x & 0x2)
#endif /* end of RTK_DRM_PLUGIN */
        ...
        case kWhatCodecNotify:
        {
            ...
            switch (what) {
                ...
                case CodecBase::kWhatBuffersAllocated:
                {
                    ...
                    int32_t ionFd = 0;
                    int32_t mapFd = 0;
                    ion_user_handle_t ionHd = 0;
                    unsigned long physAddr  = 0;
                    uint32_t physSize = 0;
                    int32_t err = 0;
                    unsigned char *virtAddress = NULL;
                    bool usedIonMemory = false;
                    int32_t addrMode = 0x3; // 0x1 --> Src Virtual , 0x2 --> Dst Virtual

                    if (portIndex == kPortIndexInput && mCrypto != NULL && numBuffers > 0)
                    {
                        uint32_t ret = 0;
                        ret = mCrypto->checkPlugin(RTK_MAGIC_NUMBER);
                        if(ret == RTK_MAGIC_NUMBER)
                        {
                            ionFd = ion_open();
                            if (ionFd < 0)
                                ALOGW("\033[0;32m In %s:%d can't open ion device \033[m", __FUNCTION__, __LINE__);
                            // debug used
                            char value[PROPERTY_VALUE_MAX];
                            if(property_get("ro.rtk.drm.addrMode", value, "0") > 0) // Always return 1 evan the property is not exist
                            {
                                addrMode = atoi(value);
                                ALOGD("\033[0;35m ---- In %s:%d Addr mode :%x \033[m", __FUNCTION__, __LINE__, addrMode);
                                ALOGV("\033[0;35m ---- Bit0 control src ptr, 1 mean virtual address, 0 mean used physical address \033[m");
                                ALOGV("\033[0;35m ---- Bit1 control dst ptr, 1 mean virtual address, 0 mean used physical address \033[m");
                            }
                        }
                    }
#endif /* end of RTK_DRM_PLUGIN */
                    ...
                    for (size_t i = 0; i < numBuffers; ++i) {
                        ...
                        int32_t srcShareFd = 0;
                        srcShareFd = 0;
                        dstShareFd = 0;
                        info.mData->meta()->findInt64("phys", &physAddrs);
                        ...
                        if (!dstShareFd)
                        {
                            if (ionFd > 0)
                            {
                                ALOGV("\033[0;32m ------------------- %s:%d %s need close ionFd:%d ---------------------\033[m", __FUNCTION__, __LINE__, mComponentName.c_str(), ionFd);
                                close(ionFd);
                                ionFd = -1;
                            }
                        }
                        ...
                        if (portIndex == kPortIndexInput && mCrypto != NULL) {
                            ...
                            if (ionFd > 0)
                            {
                                /* remove ION_FLAG_NONCACHED, arm64 did not support this flag */
                                err = ion_alloc(ionFd, info.mData->capacity(), getpagesize(), RTK_PHOENIX_ION_HEAP_MEDIA_MASK, (/* ION_FLAG_NONCACHED | */ION_FLAG_SCPUACC), &ionHd);
                                ALOGV("\033[0;32m In %s:%d ionFd:%d Used ion_alloc i:%d - total:%d size:%d return:%d ionHd:%d err:%s\033[m", __FUNCTION__, __LINE__, ionFd, (int)i, (int)numBuffers, (int)info.mData->capacity(), err, ionHd, strerror(err));
                                if (err < 0)
                                {
                                    info.mEncryptedData = new ABuffer(info.mData->capacity());
                                }
                                else
                                {
                                    err = ion_phys(ionFd, ionHd, &physAddr, &physSize);
                                    if (err < 0)
                                        ALOGD("\033[0;32m In %s:%d call ion_phys err:%d i:%d - total:%d size:%d physAddr:%llx physSize:%x mIsVideo:%d \033[m", __FUNCTION__, __LINE__, err, (int)i, (int)numBuffers, (int)info.mData->capacity(), (long long)physAddr, physSize, mIsVideo);
                                    else
                                    {
                                        err = ion_map(ionFd, ionHd, info.mData->capacity(), PROT_READ | PROT_WRITE, MAP_SHARED, 0, &virtAddress, &mapFd);
                                        if (err < 0)
                                            ALOGD("\033[0;32m In %s:%d call ion_map err:%d size:%d physAddr:%llx physSize:%x virtAddress:%p mIsVideo:%d\033[m", __FUNCTION__, __LINE__, err, (int)info.mData->capacity(), (long long)physAddr, physSize, virtAddress, mIsVideo);
                                        else
                                        {
                                            close(mapFd);
                                            usedIonMemory = true;
                                            err = ion_share(ionFd, ionHd, &srcShareFd);
                                            if (err < 0)
                                                ALOGD("\033[0;32m In %s:%d call ion_share err:%d mIsVideo:%d\033[m", __FUNCTION__, __LINE__, err, mIsVideo);
                            ...
                            if (usedIonMemory)
                            {
                                info.mEncryptedData = new ABuffer(virtAddress, info.mData->capacity());
                                info.mEncryptedData->meta()->setInt64("phyAd", physAddr);
                                info.mEncryptedData->meta()->setInt32("srcIonFd", ionFd);
                                info.mEncryptedData->meta()->setInt32("srcIonHd", ionHd);
                                info.mEncryptedData->meta()->setInt32("srcShareFd", srcShareFd);
                                info.mEncryptedData->meta()->setInt32("addrMode", addrMode);
                                ALOGD("\033[0;33m In %s:%s:%d Size:%d i:%d mData V-P:%p-0x%llx mEncryptedData V-P:%p-0x%llx mIsVideo:%d shareFd: s-%d,d-%d addrMode:%x \033[m", mComponentName.c_str(), __FUNCTION__, __LINE__, (int)numBuffers, (int)i, info.mData->data(), (long long)physAddrs, info.mEncryptedData->data(), (long long)physAddr, mIsVideo, srcShareFd, dstShareFd, addrMode);
                            }
                            else
#endif /* end of RTK_DRM_PLUGIN */
                            {
...
#ifdef RTK_DRM_PLUGIN //For Realtek DRM Plugin
        if (mState == RELEASING && portIndex == kPortIndexInput && info->mEncryptedData != NULL)
        {
            physAddr = 0;
            ionHd = 0;
            int32_t err = 0;

            info->mEncryptedData->meta()->findInt32("srcIonFd", &ionFd);
            info->mEncryptedData->meta()->findInt32("srcIonHd", &ionHd);
            info->mEncryptedData->meta()->findInt32("srcShareFd", &shareFd);
            if (ionFd > 0)
            {
                ALOGV("\033[0;33m In %s:%s:%d Size:%d i:%d mData:%p mEncryptedData:%p mIsVideo:%d\033[m", mComponentName.c_str(), __FUNCTION__, __LINE__, (int)buffers->size(), (int)i, info->mData->data(), info->mEncryptedData->data(), mIsVideo);
                info->mEncryptedData->meta()->findInt64("phyAd", &physAddr);
                if (munmap(info->mEncryptedData->base(), info->mData->capacity()) < 0)
                    ALOGD("\033[0;32m In %s:%d munmap %p, size:%d failed:%d-%s \033[m", __FUNCTION__, __LINE__, info->mEncryptedData->base(), (int)info->mData->capacity(), errno, strerror(errno));
                err = ion_free(ionFd, ionHd);
                if (err < 0)
                    ALOGD("\033[0;32m %s;%d physAddr:0x%llx ionHd:0x%x, ionFd:%d free ion_free err:%d - %s \033[m", __FUNCTION__, __LINE__, (long long)physAddr, ionHd, ionFd, err, strerror(err));
                info->mEncryptedData->meta()->clear();
            }
            if (shareFd > 0)
            {
                close(shareFd);
...
#ifdef RTK_DRM_PLUGIN // For Realtek DRM Plugin
    if (mState == RELEASING && ionFd > 0)
    {
        ion_close(ionFd);
        ALOGV("\033[0;32m In %s:%d close ion device :%d  \033[m", __FUNCTION__, __LINE__, ionFd);
        ionFd = 0;
    }
#endif /* end of RTK_DRM_PLUGIN */
...
status_t MediaCodec::onQueueInputBuffer(const sp<AMessage> &msg) {
    ...
    if (mCrypto != NULL) {
        ...
        ssize_t result = 0;
#ifdef RTK_DRM_PLUGIN // For Realtek DRM Plugin
        int64_t SrcPhyAddr = 0;
        int64_t DstPhyAddr = 0;
        int32_t ionFd = 0;
        ...
        bool usedRtkDRM = false;
        info->mEncryptedData->meta()->findInt64("phyAd", &SrcPhyAddr);
        if (SrcPhyAddr != 0)
        {
            info->mEncryptedData->meta()->findInt32("addrMode", &drmMessageData[TRANSFER_ADDR_MODE]);
            info->mEncryptedData->meta()->findInt32("srcShareFd", &drmMessageData[TRANSFER_SRC_SHAREDFD]);
            info->mEncryptedData->meta()->findInt32("srcIonFd", &ionFd);
            info->mData->meta()->findInt64("phys", &DstPhyAddr);
            info->mData->meta()->findInt32("dstShareFd", &drmMessageData[TRANSFER_DST_SHAREDFD]);
            usedRtkDRM = true;
            ALOGV("\033[0;32m $$ In %s:%d pid:%d tid:%d mIsVideo:%d - secure:%d Source- V-P :%p-0x%llx , offset:%d , Dst-V-P:%p-0x%llx shareFd -S:%d,D:%d MAX_TARNSFER_SIZE:%d\033[m", __FUNCTION__, __LINE__, getpid(), gettid(), mIsVideo, (mFlags & kFlagIsSecure), info->mEncryptedData->base(), (long long)SrcPhyAddr, (int)offset, info->mData->base(), (long long)DstPhyAddr, drmMessageData[TRANSFER_SRC_SHAREDFD], drmMessageData[TRANSFER_DST_SHAREDFD], MAX_TARNSFER_SIZE);
            ALOGV("\033[0;33m ####### In %s:%d DstPhyAddr:0x%llx SrcPhyAddr:0x%llx secure:%d shareFd S:%d D:%d addrMode:%x ######\033[m", __FUNCTION__, __LINE__, (long long)DstPhyAddr, (long long)SrcPhyAddr, (mFlags & kFlagIsSecure), drmMessageData[TRANSFER_SRC_SHAREDFD], drmMessageData[TRANSFER_DST_SHAREDFD], drmMessageData[TRANSFER_ADDR_MODE]);
            if ((ionFd > 0) && (drmMessageData[TRANSFER_SRC_SHAREDFD] > 0))
                ion_sync_fd(ionFd, drmMessageData[TRANSFER_SRC_SHAREDFD]);
        }
        if (usedRtkDRM)
        {
            result = mCrypto->decrypt(
                    ...
                    iv,
                    mode,
                    CHECK_SRC_PHYADDR(drmMessageData[TRANSFER_ADDR_MODE]) ? info->mEncryptedData->base(): (const void *)SrcPhyAddr,
                    offset,
                    subSamples,
                    numSubSamples,
                    CHECK_DST_PHYADDR(drmMessageData[TRANSFER_ADDR_MODE]) ? info->mData->base() : (void *)DstPhyAddr,
                    errorDetailMsg);
        }
        else
#endif
        {
            // Android original method
            result = mCrypto->decrypt(
                    (mFlags & kFlagIsSecure) != 0,
                    key,
                    iv,
