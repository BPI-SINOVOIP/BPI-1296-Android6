...
ARTPSource::ARTPSource(
    ...
    } else if (!strncasecmp(desc.c_str(), "MPA/", 4)) {
        mAssembler = new MPAAssembler(notify, desc.c_str(), params);
    } else if (!strncasecmp(desc.c_str(), "MPV/", 4)) {
        mAssembler = new MPVAssembler(notify, desc.c_str(), params);
#endif
    } else {
        TRESPASS();
    }
