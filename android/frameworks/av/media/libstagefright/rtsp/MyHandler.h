        ...
        if (mAllTracksHaveTime && dataReceivedOnAllChannels()) {
            ...
            for (size_t i = 0; i < mTracks.size(); ++i) {
                ...
                while (!trackInfo->mPackets.empty()) {
                    ...
                    trackInfo->mPackets.erase(trackInfo->mPackets.begin());
#ifdef RTK_PLATFORM
                    ALOGV("[%s][%d] addMediaTimestamp ++ %d A %d V %d\n",__FUNCTION__,__LINE__,(int32_t)i,(int32_t)mAudioIdx,(int32_t)mVideoIdx);
                    if(mVideoIdx != -1 && mAudioIdx != -1 &&
                            mAudioIdx == (int32_t)i && mAudioPTS > mVideoPTS &&
                            mAudioPTS - mVideoPTS > 1000ll)
                    {
                        ALOGW("[%s][%d] Audio PTS too fast break ++\n",__FUNCTION__,__LINE__);
                        continue;
                    }
                    uint32_t seqNum = (uint32_t)accessUnit->int32Data();
                    if (seqNum < track->mFirstSeqNumInSegment)
...
#ifdef RTK_PLATFORM
            if(mVideoIdx != -1 && mAudioIdx != -1 &&
                    mAudioIdx == trackIndex && mAudioPTS > mVideoPTS
                    &&  mAudioPTS - mVideoPTS > 1000ll)
            {
                ALOGW("[%s][%d] Audio PTS too fast break ++\n",__FUNCTION__,__LINE__);
                break;
