...
static int32_t getColorFormat(const char* colorFormat) {
    ...
    if (!strcmp(colorFormat, CameraParameters::PIXEL_FORMAT_YVU420SP)) {
        return OMX_RTK_COLOR_FormatYVU420SemiPlanar;
    }
...
status_t CameraSource::initWithCameraAccess(
    ...
    if (storeMetaDataInVideoBuffers) {
        ...
        ALOGV("enable store metadata in video buffers %s", mIsMetaDataStoredInVideoBuffers ? "success" : "failed");
    }
