            ...
            if (mMoofOffset > 0) {
                ...
                if (track->sampleTable->findThumbnailSample(&sampleIndex) == OK
                    ...
                    if ((err == OK) && (offset > FileSize)) // file truncated
                    {
                        ...
                        ALOGD("\033[0;33m [%d] sampleIndex= %d,  FileSize= %lld,  offset= %lld,  sampleTime= %d,  track->timescale= %d\033[m", __LINE__, sampleIndex, (long long)FileSize, (long long)offset, sampleTime, track->timescale);
                    }
                    else
#endif
                    track->meta->setInt64(
...
#ifdef RTK_PLATFORM
            ALOGD("\033[0;33m [%d] width= %d, height= %d \033[m", __LINE__, width, height);
#endif
            ...
            if (mIsTWOS || mIsSOWT)
            {
                ...
                for (unsigned int i = 0; i < mNumChunk; i++)
                {
                    ...
                    if (mIsSOWT && mOwner->mSOWTsamplesPerChunk == 0)
                    {
                        ...
                        if (err != OK)
                        {
                            ALOGD("\033[0;31m [%d]  Wrong getSOWTsamplesPerChunkSize value= %d   mCurrentSampleIndex= %d\033[m", __LINE__, stts, mCurrentSampleIndex);
                            return ERROR_END_OF_STREAM;
                        }
                        mOwner->mCurrcts += stts;
                    }
...
read_again:
        ...
        else if (mIsTWOS || mIsSOWT)
        {
            ...
            if (err == OK)
            {
                ...
                if (mIsSOWT && (mOwner->mSOWTsamplesPerChunk == 0))
                {
                    ...
                    cts = mCurrentSampleIndex * stts;
                //ALOGD("\033[0;32m [%d] mCurrentSampleIndex= %d, offset= %lld,  size= %d, cts= %d, isSyncSample= %d, stts= %d  mTimescale= %d \033[m", __LINE__, mCurrentSampleIndex, (long long)offset, (int)size, cts, isSyncSample, stts, mTimescale);
            }
        }
        else
            err = mSampleTable->getMetaDataForSample(mCurrentSampleIndex, &offset, &size, &cts, &isSyncSample, &stts);
        //ALOGD("\033[0;33m [%d] mCurrentSampleIndex= %d, offset= %lld,  size= %d, cts= %d, isSyncSample= %d, stts= %d  mTimescale= %d \033[m", __LINE__, mCurrentSampleIndex, (long long)offset, (int)size, cts, isSyncSample, stts, mTimescale);
#else
        status_t err =
    ...
    if (mDataSource->flags() & DataSource::kIsCachingDataSource) {
        ...
        if(!strncasecmp(mime, "audio/", 6)) {
            if(mOwner->mVideoOffset > 0 && offset > mOwner->mVideoOffset + 500000) {
                mBuffer->release();
                mBuffer = NULL;
