...
#define FIXED_64BIT_DIRECT_BUFFER

namespace android {

static int getProcessInfo(char * retName, int name_size, int * retPid, int * retTid);
...
status_t ACodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    if (mNativeWindow != NULL && portIndex == kPortIndexOutput) {
        ...
        if (err == OK) {
            ...
            ALOGD("[%s] Allocating %u buffers of size %d/%d (from %u using %s) on %s port",
            ...
            OMX_S32 shareFd = 0;
#ifdef FIXED_64BIT_DIRECT_BUFFER
            int32_t ionFd = 0;
            ...
            int32_t mapDstFd = 0;
            int32_t ret = 0;
#endif /* end of FIXED_64BIT_DIRECT_BUFFER */
            ...
            for (OMX_U32 i = 0; i < def.nBufferCountActual && err == OK; ++i) {
                ...
                if ((portIndex == kPortIndexInput && (mFlags & kFlagIsSecure))
                        || (portIndex == kPortIndexInput && mSideBandDst)
                        || (portIndex == kPortIndexOutput && mCameraSource)
...
#ifdef FIXED_64BIT_DIRECT_BUFFER
                    /* Get Physical address used getParameter; For Realtek DRM Plugin */
                    OMX_PLUS_EXTRADATATYPE inputHeader;
                    InitOMXParams(&inputHeader);
                    inputHeader.nPortIndex = info.mBufferID;
                    inputHeader.enableCrypto = 1;
                    status_t err = mOMX->getIonFd(mNode, OMX_IndexParamVideoInputPhysical, &inputHeader, sizeof(inputHeader), &shareFd);
                    ALOGD("\033[0;33m ======== in %s:%d %s shareFd:%d ionFd:%d i:%d def.nBufferCountActual:%d err:%x mFlags:%x reduce:%d phy:%llx OK:%x ======\033[m", __FUNCTION__, __LINE__, mComponentName.c_str(), shareFd, ionFd, (int)i, (int)def.nBufferCountActual, err, mFlags, mSideBandDst, inputHeader.nAddress, OK);
                    if (shareFd > 0 && (!mCrypto))
                    {
                        ...
                        if (dstIonHandle > 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    ALOGV("\033[0;32m ### In %s:%d phy:%lx virtual:%p ptr:%p size:%d ###\033[m", __FUNCTION__, __LINE__, physAddr, mapDstStr, ptr, physSize);
                                    ptr = mapDstStr;
                                }
                                mapDstStr = NULL;
                                if (mapDstFd > 0)
                    ...
                    if(shareFd > 0 )
                    {
                        info.mData->meta()->setInt64("phys", inputHeader.nAddress);
                        info.mData->meta()->setInt32("dstShareFd", shareFd);
                        info.mPhyAddr = inputHeader.nAddress;
                        ALOGD("\033[0;32m In %s:%s:%d physical:0x%llx shareFd:%d \033[m", mComponentName.c_str(), __FUNCTION__, __LINE__, inputHeader.nAddress, shareFd);
                        shareFd = 0;
                    }
                    else if (err == OK && mCrypto)
                    {
                        info.mData->meta()->setInt64("phys", inputHeader.nAddress);
                        info.mData->meta()->setInt32("dstShareFd", 1);
                        info.mPhyAddr = inputHeader.nAddress;
                        ALOGD("\033[0;32m In %s:%s:%d physical:0x%llx \033[m", mComponentName.c_str(), __FUNCTION__, __LINE__, inputHeader.nAddress);
                    }
#else /* else of FIXED_64BIT_DIRECT_BUFFER */
                    ...
                    InitOMXParams(&inputHeader);
                    inputHeader.nPortIndex = info.mBufferID;
                    /* Get ShareFd, need close it */
                    if (mCrypto)
                        inputHeader.enableCrypto = 1;
                    else
                        inputHeader.enableCrypto = 0;
                    status_t err = mOMX->getIonFd(mNode, OMX_IndexParamVideoInputPhysical, &inputHeader, sizeof(inputHeader), &shareFd);
                    if (err == OK)
                    {
                        info.mData->meta()->setInt64("phys", inputHeader.nAddress);
                ...
                if (mem != NULL) {
...
#if 0 /* normal case don't need phys address and shareFd */
                    if (mQuirks & requiresAllocateBufferBit)
                    {
                        ...
                        if (err == OK)
                        {
                            ...
                            info.mData->meta()->setInt32("dstShareFd", shareFd);
                            info.mPhyAddr = inputHeader.nAddress;
                            ALOGV("\033[0;32m In %s:%s:%d physical:0x%llx shareFd:%d bufferID:%d\033[m", mComponentName.c_str(), __FUNCTION__, __LINE__, inputHeader.nAddress, shareFd, (int)info.mBufferID);
                            shareFd = 0;
                        }
                    }
#endif
...
#else
	//20131230 charleslin - need to access video frame via ABuffer data pointer for transcoding
        void *data = NULL;
        err = graphicBuffer->lock(GRALLOC_USAGE_SW_READ_OFTEN, &data);
        CHECK(err == OK);
        info.mData = new ABuffer(data, bufferSize);
#endif
...
status_t ACodec::allocateOutputMetadataBuffers() {
    ...
    if (err == OK)
    {
        ...
        if (err == OK)
        {
            OMX_U32 notify = RTK_MAGIC_NUMBER;
            err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
            ALOGD("In %s:%d notifyBufferAllocDone index :0x%.8x and send notify done ", __FUNCTION__, __LINE__, index);
        }
        else
            ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.notifyBufferAllocDone\033[m");
    }
#endif /* end of CHANGE_ALLOC_BUFFER_LOCATION */
...
status_t ACodec::freeBuffer(OMX_U32 portIndex, size_t i) {
    ...
    if (info->mData->meta() != NULL)
    {
        int64_t dstInputPhyAddr = 0;
        int32_t dstShareFd = 0;
        info->mData->meta()->findInt32("dstShareFd", &dstShareFd);
        if (dstShareFd > 0 && dstShareFd != 1)
        {
            info->mData->meta()->findInt64("phys", &dstInputPhyAddr);
            ALOGV("\033[0;33m #### In %s:%d port:%s index:%d phy:0x%llx dstShareFd:%d size:%d ptr:%p \033[m", __FUNCTION__, __LINE__, portIndex == kPortIndexInput ? "input" : "output", (int)i, (long long)dstInputPhyAddr, (int)dstShareFd, (int)info->mData->capacity(), info->mData->base());
            if(!mCrypto)
                munmap(info->mData->base(), info->mData->capacity());
            close(dstShareFd);
        }
    }
...
status_t ACodec::configureCodec(
    ...
    if (video && !encoder) {
        ...
        if((encoder_width + encoder_height) > 0)
        {
            ...
            if (err == OK)
            {
                OMX_U32 encoder_size[3];
                encoder_size[0] = encoder_width;
                encoder_size[1] = encoder_height;
                encoder_size[2] = encoder_fps;
                err = mOMX->setParameter(mNode, index, encoder_size, sizeof(OMX_U32)*3);
                ALOGD("\033[0;35m In %s:%d write ve_scaling_size index:%x WxH:%dx%d scale:%d \033[m", __FUNCTION__, __LINE__, index, encoder_size[0], encoder_size[1], encoder_size[2]);
            }
            else
    ...
    if (haveNativeWindow) {
        ...
        if (msg->findInt32("feature-tunneled-playback", &tunneled) &&
            ...
            mOMX->setSideBand(mNode, mSideBandDst);
        } else {
    ...
    if (!video && !strncmp(mComponentName.c_str(), "OMX.realtek.audio.dec", 21)) {
        ...
        if (!msg->findInt32("audio-hw-sync", &audioHwSync)) {
            ...
            ALOGD("[%s %d] Audio get HW SYNC ID %d", __FUNCTION__, __LINE__, audioHwSync);
        ...
        {
            ALOGD("[%s %d] Audio get target SampleRate %d", __FUNCTION__, __LINE__, audioSampleRate);
        ...
        {
            ALOGD("[%s %d] Audio get target ChannelCount %d", __FUNCTION__, __LINE__, audioChannelCount);
    ...
    if (video) {
        ...
        if (encoder) {
...
#ifdef OMX_VIDEO_PARENT_INFO
            {
                OMX_INDEXTYPE index;
                if (mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.notifyParentInfo", &index) == OK) {
                    OMX_VIDEO_PARENT_INFO params;
                    InitOMXParams(&params);
                    getProcessInfo((char *) &params.sComm[0],
                            (int) sizeof(params.sComm),
                            (int *) &params.nPid,
                            (int *) &params.nTid);
                    if (mOMX->setParameter(mNode, index, &params, sizeof(params)) != OK)
                        ALOGD("[%s] setParameter failed! (OMX.realtek.android.index.notifyParentInfo)", __FUNCTION__);
                } else
                    ALOGI("[%s] Unsupported extension index : OMX.realtek.android.index.notifyParentInfo", __FUNCTION__);
            }
#endif
...
#else
            int32_t videoLowDelay = 0;
            if (msg->findInt32("video-lowdelay-mode", &videoLowDelay))
            {
                if (videoLowDelay)
                {
                    OMX_INDEXTYPE index;
            ...
            if(enableDeinterlace >= 0 && enableDeinterlace < 3)
            {
                ...
                if (err == OK)
                {
                    OMX_U32 notify = enableDeinterlace;
                    err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
        ...
        if ((property_get("rtk.disable.sideband", value, NULL)
                ...
                || mCrypto == true)
        {
            ALOGD("\033[0;35m in %s:%d in haveNativeWindow && disable Sideband @@@@\033[m", __FUNCTION__, __LINE__);
            SB_control = NULL;
        }
        else if ((property_get("rtk.feature-tunneled-playback", value, NULL)
            && (!strcmp("1", value) || !strcasecmp("true", value)))
            || mHackedSideband == true
/*                && (!strcasecmp(mime, "video/hevc"))*/)
        {
            ALOGD("\033[0;35m in %s:%d in haveNativeWindow && bf use Sideband @@@@\033[m", __FUNCTION__, __LINE__);
...
//        SF_surface = static_cast<Surface*>(mNativeWindow.get());//surface;//control->getSurface();
...
//static_cast<Surface*>(mNativeWindow.get()) = SB_control->getSurface();
        ...
        else
        {
            ...
            {
                mCameraSource = true;
                ALOGD("\033[0;34m in %s:%d detect no NativeWindow :%s mCameraSource:%d @@@@\033[m", __FUNCTION__, __LINE__, msg->debugString().c_str(), mCameraSource);
            }
            else
                mCameraSource = false;
        }
    ...
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC) || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC_LATM)) {
        int32_t numChannels, sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
...
//[ RTK begin ]
    ...
    if(!strncmp(mComponentName.c_str(), "OMX.realtek.video.dec", 21) && msg->findInt32("isThumbnail", &isThumbnail))
    {
        ...
        OMX_BOOL enable = (isThumbnail == 1 ? OMX_TRUE : OMX_FALSE);
...
status_t ACodec::setupDSDCodec(
        const sp<AMessage> &msg, bool encoder)
{
    int64_t sampleCount;
    int32_t parameters[5];
    char type[][32] = {"channel-count", "sample-rate", "DSD-ChType", "DSD-BlockSize", "bits-sample"};
    int i;
    status_t err= ~OK; 
    ...
    for(i=0 ; i<5 ; ++i)
    {
        if (!msg->findInt32(type[i], &parameters[i]))
        {
            err = INVALID_OPERATION;
            ALOGE("DSD:err %d @ %s %d\n", i, __func__, __LINE__);
            goto func_exit;
    ...
    if (!msg->findInt64("DSD-SampleCount", &sampleCount))
    {
        err = INVALID_OPERATION;
        ALOGE("DSD:err  @ %s %d\n", __func__, __LINE__);
        goto func_exit;
    ...
    {
        ALOGD("DSD: sample count %lld\n", sampleCount);
    }

    err = setupRawAudioFormat(
            encoder ? kPortIndexInput : kPortIndexOutput, parameters[1], parameters[0]);
    ...
    def.nChannels = parameters[0];
    def.nSampleRate = parameters[1];
    def.nChType = parameters[2];
    def.nBlock = parameters[3];
    def.nBitsPerSample = parameters[4];
    def.nSampleCount = sampleCount;
...
status_t ACodec::setupRawAudioFormat(
    ...
    pcmParams.nBitPerSample = mBitPerSample;
...
status_t ACodec::setupAVCEncoderParameters(const sp<AMessage> &msg) {
    ...
    // XXX
#if 0 //20140429 charleslin for HW video encoder
...
//[ RTK begin ]
        fmt != HAL_PIXEL_FORMAT_RTK_16_YCbCr_420_SP      &&
        fmt != HAL_PIXEL_FORMAT_RTK_32_YCbCr_420_SP      &&
        fmt != HAL_PIXEL_FORMAT_RTK_W16_H16_YCbCr_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W32_H16_YCbCr_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W16_H32_YCbCr_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W32_H32_YCbCr_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W16_H8_YCbCr_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W16_H16_YCrCb_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W32_H16_YCrCb_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W16_H32_YCrCb_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W32_H32_YCrCb_420_SP &&
        fmt != HAL_PIXEL_FORMAT_RTK_W16_H8_YCrCb_420_SP &&
//[ RTK end ]
            ...
            switch ((int)audioDef->eEncoding) {
                case OMX_AUDIO_CodingPCM:
                {
                    ...
                    if (params.nChannels <= 0
                            ...
                            || (params.nBitPerSample != 16u && params.nPortIndex == kPortIndexOutput)
                            || (params.nBitPerSample != (OMX_U32)mBitPerSample && params.nPortIndex == kPortIndexInput)
...
void ACodec::BaseState::postFillThisBuffer(BufferInfo *info) {
    ...
    if (dstShareFd != 0)
        info->mData->meta()->setInt32("dstShareFd", dstShareFd);
...
status_t ACodec::setParameters(const sp<AMessage> &params) {
    ...
    if (params->findInt32("skip-non-ref-frame", &skipRefFrame)) {
        ...
        if (err == OK)
        {
            err = mOMX->setParameter(mNode, index, &skipRefFrame, sizeof(OMX_U32));
...
int getProcessInfo(char * retName, int name_size, int * retPid, int * retTid)
{
    int ret = -1;
    do {
        int pid = (int) getpid();
        ...
        memset(retName, 0, name_size);
        sprintf(FileBuf, "/proc/%d/comm", pid);
        ...
        read(proc_fd, retName, name_size);

        if (retName[strlen(retName)-1] == '\n')
            retName[strlen(retName)-1] = '\0';

        ALOGI("[%s] %d %s ", __FUNCTION__, name_size, retName);
        close(proc_fd);
