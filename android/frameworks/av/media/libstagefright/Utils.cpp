...
status_t convertMetaDataToMessage(
    ...
    int32_t DSDBlockSize = 0;
    if (meta->findInt32(kKeyDSDBlockSize, &DSDBlockSize)) {
        msg->setInt32("DSD-BlockSize", DSDBlockSize);
    }

    int32_t DSDChType = 0;
    if (meta->findInt32(kKeyDSDChannelType, &DSDChType)) {
        msg->setInt32("DSD-ChType", DSDChType);
    }

    int64_t DSDSampleCount = 0;
    if (meta->findInt64(kKeyDSDSampleCount, &DSDSampleCount)) {
        msg->setInt64("DSD-SampleCount", DSDSampleCount);
    }
...
static const struct mime_conv_t mimeLookup[] = {
    ...
    { MEDIA_MIMETYPE_AUDIO_MLP,         AUDIO_FORMAT_TRUEHD},
