...
// When input and output of the time difference too much!
// We will pause until the next read trigger.
const static int64_t kDrainBufferIntervalTimeUs  = 3000000LL;
...
void OMXCodec::findMatchingCodecs(
    ...
    for (;;) {
        ...
        ALOGD("\033[0;32m --- In %s:%d mime:%s matchIndex:%d index:%d componentName:%s \033[m", __FUNCTION__, __LINE__, mime, (int)matchIndex, (int)index, componentName);
        ...
        if (property_get("persist.rtk.disable_enc", value, NULL) && (!strcmp(value, "1") || !strcasecmp(value, "true")))
            disableRtkEnc = true;
        ALOGD("\033[0;34m ----%s:%d Cts module disableRtkDec:%d disableRtkEnc:%d check persist.rtk.disable_dec, persist.rtk.disable_enc \033[m", __FUNCTION__, __LINE__, disableRtkDec, disableRtkEnc);
        if ((!strncmp(componentName, "OMX.realtek.video.dec", 21)) && disableRtkDec)
        {
            ALOGD("\033[0;34m ----%s:%d don't used hw decoder %s \033[m", __FUNCTION__, __LINE__, componentName);
            continue;
        }
        if ((!strncmp(componentName, "OMX.realtek.video.enc", 21)) && disableRtkEnc)
        {
            ALOGD("\033[0;34m ----%s:%d don't used hw encoder %s \033[m", __FUNCTION__, __LINE__, componentName);
            continue;
        }
#endif
...
status_t OMXCodec::parseHVCCodecSpecificData(
    ...
    tempBuf = (uint8_t *)calloc((size + 4), 1); // for insert 0x0 0x0 0x0 0x1
    if (tempBuf != NULL)
        tempBuf[3] = 1;
        ...
        *level = ptr[12];							// general_level_idc	(8bits)
        ...
        for (size_t i = 0; i < numOfArrays; i++) {
            ...
            for (size_t j = 0; j < numNalus; j++) {
                ...
                if (tempBuf != NULL)
                {
                    memcpy(tempBuf+4, ptr, nalUnitLength);
                    addCodecSpecificData(tempBuf, nalUnitLength+4);
                }
                else
                    addCodecSpecificData(ptr, nalUnitLength);
...
status_t OMXCodec::configureCodec(const sp<MetaData> &meta) {
    ...
    if (!(mFlags & kIgnoreCodecSpecificData)) {
        ...
        size_t size;
        bool checkWMV = false;
        if (meta->findData(kKeyPrivateInfo, &type, &data, &size)) {
            ALOGD("find RTK PrivateInfo size %d mMIME:%s type:%d", (int)size, mMIME, type);
            if (type == 0)
                addCodecSpecificData(data, size);
            if (mIsVideo)
            {
                if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_WMV, mMIME) || !strcasecmp(MEDIA_MIMETYPE_VIDEO_WMV3, mMIME))
                {
                    ALOGD("Setting the OMX_VIDEO_PARAM_WMVTYPE params");
                    status_t err = setWMVFormat(OMX_VIDEO_WMVFormat7);
                    if (err != OK) {
                        return err;
                    }
                    checkWMV = true;
                }
                else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_VC1, mMIME))
                {
                    ALOGV("\033[0;35m @@@@ In %s:%d detect TS & VC1 case data:%s size:%d @@@@@\033[m", __FUNCTION__, __LINE__, data, (int)size);
                    if (type == 0)
                    {
                        /* Rtk ASF profile = 0 */
                        status_t err = setWMVFormat(OMX_VIDEO_WMVFormat7);
                    ...
                    else if (!strcasecmp((const char *)data, "TSVC1"))
                    {
                        /* Rtk TS */
                        status_t err = setWMVFormat(OMX_VIDEO_RTK_TS_VC1);
                        if (err != OK) {
                            return err;
                        }
                    }
                ...
                else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_RV40, mMIME))
                {
                    ...
                    status_t err = mOMX->getParameter(mNode, OMX_IndexParamVideoRv, &paramRV, sizeof(paramRV));
                    if (err != OK) {
                        return err;
                    }
                    paramRV.eFormat = OMX_VIDEO_RVFormat9;
                    err = mOMX->setParameter(mNode, OMX_IndexParamVideoRv, &paramRV, sizeof(paramRV));
                    if (err != OK) {
                        return err;
                    }
                }
            ...
            if(!strncmp(mComponentName, "OMX.realtek.audio.dec", 21))
            {
            }
            else
            {
                /* For MXPlayer VC1 used */
                if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_WMV, mMIME))
                {
                    ALOGD("\033[0;32m ---------- In %s:%d Original codec_specific_data_size:%d change to 4 ------\033[m", __FUNCTION__, __LINE__, (int)codec_specific_data_size);
                    codec_specific_data_size = 4;
                    status_t err = setWMVFormat(OMX_VIDEO_WMVFormat7);
                    if (err != OK) {
                        return err;
                    }
            ...
            status_t err;
            const uint8_t *ptr = (const uint8_t *)data;
            ALOGE("HVCC DATA : size= %d, 0x%x 0x%x 0x%x 0x%x 0x%x", (int)size, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4]);
            if ((err = parseHVCCodecSpecificData(data, size, &profile, &level)) != OK) {
                ALOGE("Malformed HEVC codec specific data.");
                return err;
            }
            CODEC_LOGI("HEVC profile = %u , level = %u",profile, level);
            ...
            if(!strncmp(mComponentName, "OMX.realtek.audio.dec", 21))
            {
                CHECK(meta->findData(kKeyVorbisComment, &type, &data, &size));
                addCodecSpecificData(data, size);
            }
        ...
        /* For remove inband command rtkextractor */
        if(mIsVideo && !mIsEncoder && !checkWMV)
        {
            if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_WVC1, mMIME))
            {
                status_t err = setWMVFormat(OMX_VIDEO_WMVFormat8);
                if (err != OK) {
                    return err;
                }
            }
...
#ifdef RTK_AUDIO_EXTRACTOR_EN
    if(!strcasecmp(MEDIA_MIMETYPE_AUDIO_ALAC_CAFF, mMIME))
    {
        ALOGD("[mime %s @ %s %d]\n", MEDIA_MIMETYPE_AUDIO_ALAC_CAFF, __func__, __LINE__);
        setRTK_ALACFormat(meta);
    }
    else
#endif
...
//[ RTK begin ]
#ifdef USE_RT_ION
    if ((mFlags & kClientNeedsFramebuffer)
            && !strncmp(mComponentName, "OMX.realtek.video.dec", 21)) {
        OMX_INDEXTYPE index;
        status_t err =
            mOMX->getExtensionIndex(
                    mNode,
                    "OMX.RTK.index.ThumbnailMode",
                    &index);
        if (err != OK) {
            return err;
        }
        ALOGD("****** setConfig: index=0x%08X ********", (int)index);
        OMX_BOOL enable = OMX_TRUE;
        err = mOMX->setConfig(mNode, index, &enable, sizeof(enable));

        if (err != OK) {
            CODEC_LOGE("setConfig('OMX.RTK.index.ThumbnailMode') "
                       "returned error 0x%08x", err);
    ...
    /* For DvdPlayer used OMXCodec used */
    const void *isBSRingfd;
    uint32_t type;
    size_t size;

    if (meta->findData(kKeySetBSRingfd, &type, &isBSRingfd, &size))
    {
        int32_t params[8];
        memcpy(params, isBSRingfd, size);
    ...
    /* For RTKExtractor */
    int32_t isRTKExtractor;
    if(!strncmp(mComponentName, "OMX.realtek.video.dec", 21) && meta->findInt32(kKeyRTKExtr, &isRTKExtractor))
    {
        OMX_INDEXTYPE index;
        status_t err =
            mOMX->getExtensionIndex(
        ...
        if (err != OK)
            return err;
        ALOGD("****** setConfig: index=0x%08X ********", (int)index);
        OMX_BOOL enable = (isRTKExtractor == 1 ? OMX_TRUE : OMX_FALSE);
        err = mOMX->setConfig(mNode, index, &enable, sizeof(enable));

        if (err != OK)
        {
            CODEC_LOGE("setConfig('OMX.RTK.index.ExtractorMode') returned error 0x%08x", err);
            return err;
        }
    }
    ...
    if (mIsVideo && !mIsEncoder)
    {
        ...
        if (meta->findInt32(kKeyVideoLowDelay, &videoLowDelay))
        {
            if (videoLowDelay)
            {
                OMX_INDEXTYPE index;
                status_t err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.videoLowDelayMode", &index);
                if (err == OK)
                {
        ...
        {
            ALOGD("\033[0;35m In %s:%d not detect video low delay mode \033[m", __FUNCTION__, __LINE__);
        }
    }
    ...
    if (mIsVideo && !mIsEncoder)
    {
        ...
        char value[PROPERTY_VALUE_MAX];
        meta->findInt32(kKeyVideoSearchIFrame, &searchIFrame);
        if (searchIFrame)
        ...
        {
            OMX_INDEXTYPE index;
            status_t err = mOMX->getExtensionIndex(mNode, "OMX.realtek.android.index.setSearchIFrm", &index);
            if (err == OK)
            {
                OMX_U32 notify = index;
                index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
...
static size_t getFrameSize(
    ...
    switch (colorFormat) {
        ...
        case OMX_RTK_COLOR_FormatYVU420SemiPlanar:
...
status_t OMXCodec::findTargetColorFormat(
    ...
    *colorFormat = OMX_COLOR_FormatYUV420SemiPlanar;
    ALOGV("set default color format:0x%x", *colorFormat);
    int32_t targetColorFormat;
    if (meta->findInt32(kKeyColorFormat, &targetColorFormat)) {
        *colorFormat = (OMX_COLOR_FORMATTYPE) targetColorFormat;
        ALOGV("set color format:0x%x", *colorFormat);
    }
...
status_t OMXCodec::setupMPEG4EncoderParameters(const sp<MetaData>& meta) {
    ...
    CHECK_EQ(setupBitRate(bitRate), (status_t)OK);
    CHECK_EQ(setupErrorCorrectionParameters(), (status_t)OK); //20131017 charleslin - comment this line may fix some issue
...
status_t OMXCodec::setupAVCEncoderParameters(const sp<MetaData>& meta) {
    ...
    // XXX
#if 0 //20160623 charleslin for HW video encoder
...
status_t OMXCodec::setVideoOutputFormat(
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) {
    ...
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_RV30, mime)) {
        compressionFormat = OMX_VIDEO_CodingRV;
    } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_RV40, mime)) {
        compressionFormat = OMX_VIDEO_CodingRV;
        ...
        CODEC_LOGV("HEVC selected (compression format = OMX_VIDEO_CodingHEVC)");
        compressionFormat = (OMX_VIDEO_CODINGTYPE) OMX_VIDEO_CodingHEVC;
        ...
        compressionFormat = OMX_VIDEO_CodingSORENSON;
    } else {
        ALOGE("Not a supported video mime type: %s", mime);
        CHECK(!"Should not be here. Not a supported video mime type.");
    }
...
#ifdef RTK_AUDIO_EXTRACTOR_EN
        { MEDIA_MIMETYPE_AUDIO_ALAC_CAFF,
            "audio_decoder.alac_caff", NULL },
#endif
//rtk end
    };
...
status_t OMXCodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    if ((portIndex == kPortIndexInput) && (mFlags & kUseSecureInputBuffers))
    {
        ...
        if (err == OK)
        {
            ...
            index = (OMX_INDEXTYPE)OMX_IndexParamConsumerUsageBits;
            err = mOMX->setParameter(mNode, index, &notify, sizeof(notify));
            ALOGD("In %s:%d notifyProtectedMode index :0x%.8x and send notify done ", __FUNCTION__, __LINE__, index);
        }
        else
            ALOGW("\033[0;32m Decoder don't understand this command : OMX.realtek.android.index.notifyProtectedMode\033[m");
    }
#endif /* end of ENABLE_TEE_DRM_FLOW */
    ...
    if (portIndex == kPortIndexInput && (mFlags & kUseSecureInputBuffers)) {
        ...
        for (size_t i = 0; i < def.nBufferCountActual; ++i) {
            ...
            /* Get Physical address used getParameter */
            OMX_BUFFERHEADERTYPE inputHeader;
            InitOMXParams(&inputHeader);
            inputHeader.nInputPortIndex = info.mBuffer;
            status_t err = mOMX->getParameter(mNode, OMX_IndexParamVideoInputPhysical, &inputHeader, sizeof(inputHeader));
            ALOGD("\033[0;32m In %s:%d info.mBuffer:0x%x, Virtual:0x%p physical:0x%llx size:%d err:%x\033[m", 
            __FUNCTION__, __LINE__, info.mBuffer, inputHeader.pBuffer, (long long)inputHeader.pPlatformPrivate, inputHeader.nAllocLen, err);
            if (err == OK)
                mbuf->meta_data()->setInt32(kKeyPlatformPrivate, (int32_t)inputHeader.pPlatformPrivate);
...
status_t OMXCodec::allocateOutputBuffersFromNativeWindow() {
...
#if 0 //RTK begin - FIXME: replaced with setNativeWindowSizeFormatAndUsage()?
    /* Reduce memory usage */
    if (mState != RECONFIGURING)
    {
#if 1 //Fuchun 20141006
        err = native_window_set_buffers_geometry(
                mNativeWindow.get(),
                32 /*def.format.video.nFrameWidth*/,
                32 /*def.format.video.nFrameHeight*/,
...
#endif
        ALOGD("\033[0;33mIn %s:%d configureOutputBuffersFromNativeWindow: %lux%lu fmt:%d size:%lu\033[m", __FUNCTION__, __LINE__, def.format.video.nFrameWidth, def.format.video.nFrameHeight, def.format.video.eColorFormat, def.nBufferSize);
    }
    else
    {
        err = native_window_set_buffers_geometry(
    ...
    for (OMX_U32 extraBuffers = 0/*2 + 1*/; /* condition inside loop */; extraBuffers--) {
...
void OMXCodec::on_message(const omx_message &msg) {
    ...
    switch (msg.type) {
        ...
        case omx_message::FILL_BUFFER_DONE:
        {
            ...
            /* Timing issue, Got buffer after freeBuffer */
            if (i >= buffers->size())
            {
                ALOGW("\033[0;33mBuffer index:%d, available buffer count :%d workaround avoid abort\033[m", (int)i, (int)buffers->size());
                return;
            }
            CHECK(i < buffers->size());
            BufferInfo *info = &buffers->editItemAt(i);
            ...
            if (mPortStatus[kPortIndexOutput] == DISABLING) {
                ...
                ALOGV("FILL_BUFFER_DONE %s timestamp:%f", mIsVideo ? "video" : "audio", msg.u.extended_buffer_data.timestamp / 1000000.0);
                ...
                mLastOutputTimeUs = msg.u.extended_buffer_data.timestamp;

                if (mPauseTargetTimeUs != -1 && mPauseTargetTimeUs <= mLastOutputTimeUs) {
                    mPauseTargetTimeUs = -1;
                }
        ...
        case OMX_CommandFlush:
        {
            ...
            if (mState == RECONFIGURING) {
                ...
                if (mPortStatus[kPortIndexInput] == ENABLED
                    ...
                    /*Fuchun 20130529 need send metadata again*/
                    //if(!strncmp(mComponentName, "OMX.realtek.video.dec", 21))
                    mCodecSpecificDataIndex = 0;
...
void OMXCodec::fillOutputBuffers() {
    ...
    if (mSignalledEOS
        ...
        mNoMoreOutputData = true;
        mBufferFilled.signal();
        ALOGD("[%s:%d] Set stop by fillOutputBuffers workaround", __FUNCTION__, __LINE__);
        return;
    }
#endif
    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexOutput];
...
bool OMXCodec::drainInputBuffer(BufferInfo *info) {
    ...
    if (mCodecSpecificDataIndex < mCodecSpecificData.size()) {
        ...
        if ((!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mMIME) ||
            ...
            //CODEC_LOGV("CodecSpecificData[%d/%d] size:%d 00 00 00 01 %02x", mCodecSpecificDataIndex, mCodecSpecificData.size(), specific->mSize, specific->mData[0]);
            memcpy(info->mData, kNALStartCode, 4);
            memcpy((uint8_t *)info->mData + 4,
                   specific->mData, specific->mSize);
        }

#if 0   /* Marked below on 20141118. It caused xbmc playback HEVC seq info initial failed. */
        else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_HEVC, mMIME)
                && !(mQuirks & kWantsNALFragments)) {
    ...
    else if (//!mFilledBuffers.empty() &&
             !mIsEncoder && //20131017 charleslin - software h264 encoder is too slow and will stop output data
             (mLastInputTimeUs-mLastOutputTimeUs) > kDrainBufferIntervalTimeUs &&
             mLastOutputTimeUs   > 0 &&
             mLastInputTimeUs    > 0 &&
             mPauseTargetTimeUs == -1) {
        CODEC_LOGV("Input and output of the time difference too much! (In:%.3f Out:%.3f)",
            (float)mLastInputTimeUs/1E6,(float)mLastOutputTimeUs/1E6);
        mPaused = true;
        mPausedDone = true;
        mBufferFilled.signal();
        return false;
    ...
    for (;;) {
        ...
        if (mSeekTimeUs >= 0) {
            ...
            //ALOGD("drainInputBuffer: read buffer");
            err = mSource->read(&srcBuffer);
        }

        if (err != OK) {
			if (err == INFO_FORMAT_CHANGED && !mIsVideo && mIsEncoder)
			{
				ALOGV("---- In [%d] err [%d] mIsEncoder [%d] mIsVideo [%d] ----", __LINE__, err, mIsEncoder, mIsVideo);
				continue;
			}
            signalEOS = true;
            mFinalStatus = err;
...
#if 1
        if(mIsEncoder && mInitialReadTimeUs == 0){
            sp<MetaData> format = mSource->getFormat();
            if(format->findInt64(kKeyInitialReadTime, &mInitialReadTimeUs)){
                mOutputFormat->setInt64(kKeyInitialReadTime, mInitialReadTimeUs);
                ALOGD("got %s initial read time:%f", mIsVideo ? "video" : "audio", mInitialReadTimeUs / 1000000.0);
            }
        }
#endif
...
#if 0
            if(mIsVideo){
                static Statistics *st = new Statistics("OMXvinbps", 5);
                st->rate(srcBuffer->range_length() * 8);
            }else{
                static Statistics *st = new Statistics("OMXainbps", 5);
                st->rate(srcBuffer->range_length() * 8);
            }
#endif
        }
        ...
        ALOGV("EMPTY_BUFFER_DONE %s timestamp:%f", mIsVideo ? "video" : "audio", lastBufferTimeUs / 1000000.0);
        ...
        if (releaseBuffer) {
            //ALOGD("drainInputBuffer: release buffer");
            srcBuffer->release();
            srcBuffer = NULL;
        }
    ...
    if ((timestampUs - mLastInputTimeUs) >= kDrainBufferIntervalTimeUs) {
        mPauseTargetTimeUs = timestampUs;
        CODEC_LOGV("PTS gap is too large! mPauseTargetTimeUs = %.3f secs",(float)mPauseTargetTimeUs/1E6);
    }

    mLastInputTimeUs = timestampUs;
...
#ifdef RTK_AUDIO_EXTRACTOR_EN
status_t OMXCodec::setRTK_ALACFormat(const sp<MetaData>& meta )
{
    OMX_AUDIO_PARAM_ALACTYPE profile;
    ALACSpecificConfig *pALACConfig;
    int32_t numChannels, sampleRate, bitRate;
    status_t err;

    InitOMXParams(&profile);

    err = mOMX->getParameter(mNode, OMX_IndexParamAudioAlac, &profile, sizeof(profile));
    CHECK_EQ(err, (status_t)OK);

    CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
    CHECK(meta->findInt32(kKeySampleRate, &sampleRate));
    CHECK(meta->findInt32(kKeyBitRate, &bitRate));
    CHECK(meta->findPointer(kKeyALAC_CAFFConfig, (void **)&pALACConfig));

    profile.nChannels = numChannels;
    profile.nSamplingRate = sampleRate;
    profile.nBitRate = bitRate;
    memcpy(&profile.nConfig, pALACConfig, sizeof(*pALACConfig));
    ALOGD("[ALAC send cookie from StageFright to RtkOmx @ %s %d]\n", __func__, __LINE__);

    err = mOMX->setParameter(mNode, OMX_IndexParamAudioAlac, &profile, sizeof(profile));

    if (err != OK)
    {
        CODEC_LOGE("setParameter('OMX_IndexParamAudioAlac') failed "
            "(err = %d)", err);
    }
    return err;
}
...
#ifdef MS12_FLOW_EN
status_t OMXCodec::setMS12FlowConfig(uint32_t cfg_addr){
    ALOGE(" %s\n", __FUNCTION__);
    printf("[YLLIN][setMS12FlowConfig]start\n");
    //OMX_AUDIO_PARAM_MS12CFG *cfg = NULL;
    //cfg = (OMX_AUDIO_PARAM_MS12CFG*)malloc(sizeof(OMX_AUDIO_PARAM_MS12CFG));
    //printf("[YLLIN][setMS12FlowConfig] %x %x\n",cfg_addr,cfg);

    OMX_AUDIO_PARAM_MS12CFG* inCfg = (OMX_AUDIO_PARAM_MS12CFG*)cfg_addr;
    //cfg.type = inCfg->type;//inCfg->type;
    //    memcpy(cfg,inCfg,sizeof(OMX_AUDIO_PARAM_MS12CFG));
    //printf("[YLLIN][setMS12FlowConfig] type %d %d\n",inCfg->type,cfg->type);
    status_t err = mOMX->setParameter(mNode, OMX_IndexParamAudioMS12, inCfg, sizeof(OMX_AUDIO_PARAM_MS12CFG));
    CHECK_EQ(err, (status_t)OK);
    //    if(cfg) free(cfg);
    printf("[YLLIN][setMS12FlowConfig] end\n");
...
//video
status_t OMXCodec::setWMVFormat(OMX_VIDEO_WMVFORMATTYPE wmvFormat)
{
    ...
    status_t err = mOMX->getParameter(mNode, OMX_IndexParamVideoWmv, &paramWMV, sizeof(paramWMV));
    if (err != OK)
        return err;

    paramWMV.eFormat = wmvFormat;

    err = mOMX->setParameter(mNode, OMX_IndexParamVideoWmv, &paramWMV, sizeof(paramWMV));
    return err;
}
...
status_t OMXCodec::read(
    ...
    *buffer = NULL;
    ...
    #ifndef DISABLE_RTKTUNING
    if (mPaused && !seeking) {
        if ((mLastInputTimeUs - mLastOutputTimeUs) < kDrainBufferIntervalTimeUs){
            mPaused = false;
            drainInputBuffers();
        }
    }
    ...
    if (seeking) {
        ...
        /* For the component need send metadata again case.
         * Flow:
         *  1. Pause: Set mPaused to true.
         *  2. Flush component and set mCodecSpecificDataIndex to 0.
         *  3. DrainInputBuffer
         *      a. Send SpecificData.
         *      b. Send Pktdata.
         */
        ...
        if (countBuffersWeOwn(mPortBuffers[kPortIndexInput]) != 0) {
            while (!mPausedDone && !mSignalledEOS) {
    ...
    if (mFilledBuffers.empty()) {
        CODEC_LOGW("mFilledBuffers.empty!! Input %d | Output %d",
            countBuffersWeOwn(mPortBuffers[kPortIndexInput]),
            countBuffersWeOwn(mPortBuffers[kPortIndexOutput]));
    }
    #endif
    while (mState != ERROR && !mNoMoreOutputData && mFilledBuffers.empty()) {
		/* For encoder dead lock issue */
		if (mIsEncoder && mSignalledEOS)
		{
			CODEC_LOGV("@@ In [%d] Encoder test EOS , we need break this for loop isVideo [%d] @@", __LINE__, mIsVideo);
			break;
		}
#if 1
        ...
        if(!mIsVideo){
            t1 = systemTime();
            CODEC_LOGV("%s:%d before %.3fsec", "read", __LINE__, (t1 - t2)/1000000000.0);
        }
#endif
        if ((err = waitForBufferFilled_l()) != OK) {
            return err;
        ...
        if(!mIsVideo){
            t2 = systemTime();
            CODEC_LOGV("%s:%d waitForBufferFilled %.3fsec", "read", __LINE__, (t2 - t1)/1000000000.0);
        }
#endif
    }
...
void OMXCodec::initOutputFormat(const sp<MetaData> &inputFormat) {
    ...
    switch (def.eDomain) {
        ...
        case OMX_PortDomainVideo:
        {
            ...
            } else if (video_def->eCompressionFormat == (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingHEVC) {
                mOutputFormat->setCString(
            ...
            //ALOGV("initOutputFormat %dx%d", video_def->nFrameWidth, video_def->nFrameHeight);
