...
void PlaylistFetcher::onMonitorQueue() {
    ...
    if (finalResult == OK &&( bufferedDurationUs < kMinBufferedDurationUs
            || bufferedAudioDurationUs < kMinBufferedAudioDurationUs))
#else
    if (finalResult == OK && bufferedDurationUs < kMinBufferedDurationUs)
#endif
    {
    ...
    if(buffer->size() > 0)
    {
        ...
        for (index = 0; index <= (int)buffer->size() - (int)(3 * PacketSize); index++)
        {
            if((packetTempS[index] == 0x47) && (packetTempS[index+ PacketSize] == 0x47) && (packetTempS[index+ PacketSize*2] == 0x47))
            {
                if(index > 0)
                {
                    ALOGD("%s find next sync word index::%d.\n", __FUNCTION__, index);

                    if(buffer->offset() + buffer->size() > buffer->capacity())
                        return -1;
                    buffer->setRange(buffer->offset() + index,(buffer->size() - index));
                }
                break;
            }
        }
...
//        mFirstPTSValid = false;
    ...
    while (offset + 188 <= buffer->size()) {
        ...
        if(err==BAD_VALUE) {
            ...
            for (int index = 0; offset + index <= buffer->size() - 3*PacketSize; index++) {
                if((packetTempS[index] == 0x47) && (packetTempS[index+ PacketSize] == 0x47) && (packetTempS[index+ PacketSize*2] == 0x47)){
...
int32_t PlaylistFetcher::checkStatus()
{
    ...
    if(mStatusFlowInfo == kPlaylistStatus_FetchFile && StatusTakeDurTime() > kPlaylistOverTime)
    {
        ALOGE("[%s][%d] kPlaylistStatus_FetchFile LineInfo:: %d overtime::%d need reconnect.\n",__FUNCTION__,__LINE__, mStatusLineInfo, StatusTakeDurTime());
        return kPlaylistStatus_FetchFileBlocked;
    }
    if(mStatusFlowInfo == kPlaylistStatus_initDownloadState && StatusTakeDurTime() > kPlaylistOverTime)
    {
        ALOGE("[%s][%d] kPlaylistStatus_initDownloadState LineInfo:: %d overtime::%d need reconnect.\n",__FUNCTION__,__LINE__, mStatusLineInfo, StatusTakeDurTime());
        return kPlaylistStatus_FetchStatusError;
    }
    if(mStatusFlowInfo != kPlaylistStatus_Normal && StatusTakeDurTime() > (kPlaylistOverTime / 2))
    {
...
unsigned int PlaylistFetcher::StatusTakeDurTime()
{
