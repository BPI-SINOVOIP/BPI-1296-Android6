...
//#define ENABLE_TEST_FREE_RUN   true       /* if true, then print the read delay */
//#define RTK_PTS_LOG                       /* Performance debug */
namespace android {

static int64_t kLowWaterMarkUs = 2000000ll;  // 2secs
static int64_t kHighWaterMarkUs = 10000000ll;  // 5secs => 10secs
...
#define DCRT_CMD_WAIT_VSYNC    DC2VO_WAIT_FOR_VSYNC
static int64_t kHighWaterMarkSeekUs =     5000000;  //    5 secs
static int64_t kLowWaterMarkSeekUs  =      500000;  //  0.5 secs
static int64_t SeekTimeIntervalUs   =     3000000;  //    3 secs
static int64_t SeekTimeStartUs;
static int64_t SeekPauseTimeUs = -1;
/*      | `
 * High |    `
 * Water|      `
 * Mark |         `
...
typedef struct {
    ...
    int64_t video_pts_delta;
} pts_unit_info;
static pts_unit_info pts_info_queue[MAX_PTS_LOG];
static int pts_start = 0;

void PTS_log_init(void)
{
    memset(&pts_info_queue[0],0,sizeof(pts_unit_info)*MAX_PTS_LOG);
    pts_start = 0;
}
void PTS_log(pts_unit_info *pts_unit)
{
    memcpy(&pts_info_queue[pts_start],pts_unit,sizeof(pts_unit_info));

    if(pts_start == 0){
        pts_info_queue[pts_start].system_time_delta =
            pts_info_queue[pts_start].system_time
             - pts_info_queue[MAX_PTS_LOG-1].system_time ;

        pts_info_queue[pts_start].video_pts_delta =
            pts_info_queue[pts_start].video_pts
             - pts_info_queue[MAX_PTS_LOG-1].video_pts ;
    }else{
        pts_info_queue[pts_start].system_time_delta =
            pts_info_queue[pts_start].system_time
             - pts_info_queue[pts_start-1].system_time ;

        pts_info_queue[pts_start].video_pts_delta =
            pts_info_queue[pts_start].video_pts
             - pts_info_queue[pts_start-1].video_pts ;
    }

    int i;
    int k = pts_start;
    pts_start ++;
    if(pts_start >= MAX_PTS_LOG) pts_start = 0;

    #define PRINT_PTS_INIT                                                                              \
    ALOGI("+--------+------------+------------+-----------+-----------+------------+--------------+");  \
    ALOGI("|        | Video PTS  | Audio PTS  | STD (us)  | Read (us) | LatenessUs | video_buffer |");  \
    ALOGI("+--------+------------+------------+-----------+-----------+------------+--------------+");

    #define PRINT_PTS_END                                                                               \
    ALOGI("+--------+------------+------------+-----------+-----------+------------+--------------+");

    #define PRINT_PTS(i)                                                                                \
    ALOGI("| %s | %-10lld | %-10lld | %9lld | %9lld | %10lld | 0x%08x   |"                             \
                                ,(pts_info_queue[i].cmd == PTS_SEEK)?       " SEEK ":                   \
                                 (pts_info_queue[i].cmd == PTS_DROP)?       " DROP ":                   \
                                 (pts_info_queue[i].cmd == PTS_RENDER)?     "RENDER":"unknow"           \
                                , pts_info_queue[i].video_pts                                           \
                                , pts_info_queue[i].audio_pts                                           \
                                , pts_info_queue[i].system_time_delta                                   \
                                , pts_info_queue[i].read_time                                           \
                                , pts_info_queue[i].latenessUs                                          \
                                , pts_info_queue[i].video_buffer                                        \
                                 );

    if (pts_unit->cmd == PTS_SEEK) {
        PRINT_PTS_INIT;
        for (i = pts_start;i < MAX_PTS_LOG;i++) { PRINT_PTS(i); }
        for (i = 0;i < pts_start;i++)           { PRINT_PTS(i); }
        PRINT_PTS_END;
    }

    #if 0
    if(pts_info_queue[k].video_pts_delta == 0 && pts_info_queue[i].video_pts != 0){
        PRINT_PTS_INIT;
        int j = k-1;
        if(j < 0 ) j = MAX_PTS_LOG-1;
        PRINT_PTS(j)
        PRINT_PTS(k)
        PRINT_PTS_END;
    }
...
AwesomePlayer::AwesomePlayer()
    ...
    #if !defined(DISABLE_RTKTUNING)
	mdcrt_fd = open("/dev/graphics/fb0", O_RDWR);
	if(mdcrt_fd == -1){
		ALOGE("ERROR Open /dev/dcnohw\n");
	}
    #endif
...
void AwesomePlayer::reset_l() {
    ...
    mSeekTimeUs = 0;
    #ifndef DISABLE_RTKTUNING
    mSeekThresholdUs = kHighWaterMarkSeekUs;
    SeekTimeStartUs  = 0;
    SeekPauseTimeUs = -1;
    #else
    mSeekThresholdUs = 500000;
    #endif /* End of DISABLE_RTKTUNING */
...
void AwesomePlayer::onBufferingUpdate() {
        ...
        if (eos) {
            ...
            if (getCachedDuration_l(&cachedDurationUs, &eos2) && mDurationUs > 0) {
                ...
                if ((mFlags & PLAYING) && !eos
                        && (cachedDataRemaining < (kLowWaterMarkBytes + kHighWaterMarkBytes)/2)
    ...
    if (getCachedDuration_l(&cachedDurationUs, &eos)) {
        ...
        #ifndef DISABLE_RTKTUNING
        if ((mFlags & PLAYING) && !eos
                && (cachedDurationUs < (kHighWaterMarkUs + kLowWaterMarkUs)/2)
...
status_t AwesomePlayer::play_l() {
    if (mFlags & SEEK_PREVIEW){
        modifyFlags(AFTER_SEEK_PREVIEW, SET);
    }
...
status_t AwesomePlayer::getPosition(int64_t *positionUs) {
    if (mSeeking != NO_SEEK) {
        ...
        *positionUs = mVideoTimeUs;
        ...
        #if !defined(DISABLE_RTKTUNING)
        if (mVideoSource != NULL && !(mFlags & VIDEO_AT_EOS)) {
            Mutex::Autolock autoLock(mMiscStateLock);
            *positionUs = mVideoTimeUs;
        }else
        #endif
...
status_t AwesomePlayer::seekTo_l(int64_t timeUs) {
    ...
    if (!(mFlags & PLAYING)) {
        ...
        if ((mFlags & PREPARED) && mVideoSource != NULL) {
            ...
            modifyFlags(AFTER_SEEK_PREVIEW, CLEAR);
            postVideoEvent_l();
        }
    }
...
void AwesomePlayer::finishSeekIfNecessary(int64_t videoTimeUs) {
    ...
    if (mAudioPlayer != NULL) {
        ...
        #ifdef DISABLE_RTKTUNING
        #ifndef USE_RTK_EXTRACTOR
        mAudioPlayer->seekTo(videoTimeUs < 0 ? mSeekTimeUs : videoTimeUs);
        #else
        //if(mFileSource == NULL || mFileSource->RTK_EXT == 0)
            mAudioPlayer->seekTo(videoTimeUs < 0 ? mSeekTimeUs : videoTimeUs);
...
void AwesomePlayer::onVideoEvent() {
    ...
    #ifndef DISABLE_RTKTUNING
    /* We're triggering a seek event to RTKExtractor
     * after preview seek. */
    if (    (mFlags & AFTER_SEEK_PREVIEW)
        &&  (mSeeking == NO_SEEK)) {
        mSeeking = SEEK;
        mSeekTimeUs = mLastVideoTimeUs;
        mSeekThresholdUs = kHighWaterMarkSeekUs;
    ...
    if (wasSeeking != NO_SEEK) {
        ...
        #ifndef DISABLE_RTKTUNING
        if (mVideoSource != NULL
            &&(     (wasSeeking == SEEK && isStreamingHTTP()) )){
            mVideoSource->pause();
        }
    ...
    if (!mVideoBuffer) {
        ...
        if (mSeeking != NO_SEEK) {
            ...
            #ifdef USE_RTK_EXTRACTOR
            if (mFileSource != NULL && mFileSource->RTK_EXT != 0){
                /* Using the flag "ReadOptions::SEEK_NEXT_SYNC" 
                 * for RTKExtractor to detect the single seek case. */
                options.setSeekTo(
                        mSeekTimeUs,
                        (mFlags & (SEEK_PREVIEW /* |AFTER_SEEK_PREVIEW */))
            ...
            options.setSeekTo(
                    mSeekTimeUs,
                    wasSeeking == SEEK_VIDEO_ONLY
                        ? MediaSource::ReadOptions::SEEK_NEXT_SYNC
                        : MediaSource::ReadOptions::SEEK_CLOSEST_SYNC);
            #ifndef DISABLE_RTKTUNING
            if (mAudioPlayer != NULL ) {
                mAudioPlayer->seekTo(mSeekTimeUs);
                if (!(mFlags & (AUDIO_RUNNING | SEEK_PREVIEW))) {
                    status_t err = startAudioPlayer_l();
                    if (err != OK) {
                        ALOGE("[%s %d] Starting the audio player failed w/ err %d",__FUNCTION__,__LINE__,err);
                        return;
                    }
                }
            }
        ...
        for (;;) {
            ...
            options.clearSeekTo();
            long long delta_time = (systemTime(SYSTEM_TIME_MONOTONIC)/1000) - start_time;
            pts_info.read_time = delta_time;
            #ifdef ENABLE_TEST_FREE_RUN
            if(delta_time > 20000 && ENABLE_TEST_FREE_RUN) {
                ALOGE("--------------------------------------------------------------------------");
                //for(int i = 0; i < (delta_time/10000); i++)
                ALOGE("[%s %d] read video buffer much too late %lld (us) mVideoBuffer = 0x%08x\n"
                                                ,__FUNCTION__,__LINE__,delta_time,mVideoBuffer);
                ALOGE("--------------------------------------------------------------------------");
              }
            #endif //End of ENABLE_TEST_FREE_RUN
            #else
            status_t err = mVideoSource->read(&mVideoBuffer, &options);
            options.clearSeekTo();
            #endif //End of RTK_PTS_LOG
    ...
    #ifdef RTK_PTS_LOG
    else pts_info.read_time = 99999;

    pts_info.video_buffer = (unsigned int)mVideoBuffer;
    #endif //End of RTK_PTS_LOG

    #ifndef DISABLE_RTKTUNING
    /* Reduce the queue buffers in OMXCodec, for Stream case. */
    if ((mFlags & SEEK_PREVIEW) && isStreamingHTTP() && mVideoSource!= NULL) {
        mVideoSource->pause();
    }
    #endif /* End of DISABLE_RTKTUNING */
    ...
    #ifndef DISABLE_RTKTUNING
    /* wait audio ready. */
    if (wasSeeking != NO_SEEK && !(mFlags & AUDIO_AT_EOS) && mAudioPlayer != NULL && (mFlags & AUDIO_RUNNING)) {
        int counter = 0;
        int64_t realTimeUs, mediaTimeUs;

        /* wait audio seek */
        while(mAudioPlayer->isSeeking() && counter++ <= 100)
            usleep(10000);

        /* wait audio seek done */
        while (!mAudioPlayer->getMediaTimeMapping(&realTimeUs, &mediaTimeUs) && counter++ <= 100)
            usleep(10000);

        if (counter >= 100) {
            ALOGE("[%s %d] ERROR! Audio always seeking!",__FUNCTION__,__LINE__);
            return;
        }
    }
    #endif /* End of DISABLE_RTKTUNING */
    ...
    #ifndef DISABLE_RTKTUNING
    TimeSource *ts =
        ((mFlags & AUDIO_AT_EOS) || !(mFlags & AUDIOPLAYER_STARTED) || (wasSeeking != NO_SEEK) || (SeekPauseTimeUs != -1))
            ? &mSystemTimeSource : mTimeSource;
    #else /* else of DISABLE_RTKTUNING */
    TimeSource *ts =
    ...
    #ifdef RTK_PTS_LOG
    pts_info.video_pts = timeUs;
    pts_info.audio_pts = mediaTimeUs;
    pts_info.system_time =  ts->getRealTimeUs();
    #endif //End of RTK_PTS_LOG
    ...
    #ifndef DISABLE_RTKTUNING
    if (wasSeeking != NO_SEEK) {
        SeekTimeStartUs = mSeekTimeUs?timeUs:0;
        ALOGV("[%s %d] SeekTimeStartUs = %.3f sec",__FUNCTION__,__LINE__,
                (float)SeekTimeStartUs/1E6);
        ...
        //latenessUs = 0; //Close The SeekPause, for Test

        if ( latenessUs > kLowWaterMarkSeekUs && mAudioPlayer != NULL && (mFlags & AUDIO_RUNNING)) {
            mAudioPlayer->pause();
            int64_t SeekPauseDelayUs = - mAudioPlayer->getLatencyUs();
            modifyFlags(AUDIO_RUNNING, CLEAR);
            mTimeSourceDeltaUs = ts->getRealTimeUs() - timeUs;

            /* timeUs + latenessUs  : Audio PTS
             * SeekPauseDelayUs     : Fixed the Audio resume play timing.
             */
            SeekPauseTimeUs = timeUs + latenessUs + SeekPauseDelayUs;
            if (latenessUs > 15000000ll) {
                ALOGE("[%s %d] SeekPauseTimeUs = %.3f !!!! Please check it!!!! (T:%.3f + L:%.3f + D:%.3f)",
                    __FUNCTION__,__LINE__,
                    (float)SeekPauseTimeUs/1E6,
                    (float)timeUs/1E6,
                    (float)latenessUs/1E6,
                    (float)SeekPauseDelayUs/1E6);
                SeekPauseTimeUs = timeUs + SeekPauseDelayUs;
            }
            ALOGI("[%s %d] SeekPauseTimeUs = %.3f secs (T:%.3f + L:%.3f + D:%.3f)",__FUNCTION__,__LINE__,
                (float)SeekPauseTimeUs/1E6,
                (float)timeUs/1E6,
                (float)latenessUs/1E6,
                (float)SeekPauseDelayUs/1E6);

            /* Correct the parameter mTimeSourceDeltaUs */
            mTimeSourceDeltaUs = ts->getRealTimeUs() - timeUs;
        } else {
            SeekPauseTimeUs = -1;
            ALOGI("[%s %d] SeekPauseTimeUs = %lld",__FUNCTION__,__LINE__,SeekPauseTimeUs);
        }
    }
    #endif /* End of DISABLE_RTKTUNING */
    ...
    if (wasSeeking == NO_SEEK) {
        ...
        #ifndef DISABLE_RTKTUNING
        if (SeekPauseTimeUs != -1) {
            if (timeUs >= (SeekPauseTimeUs)) {
                SeekPauseTimeUs = -1;
                SeekTimeStartUs = timeUs;
                mSeekThresholdUs = kHighWaterMarkSeekUs;
            }
        ...
        #ifdef RTK_PTS_LOG
        pts_info.latenessUs = latenessUs;
        #endif //End of RTK_PTS_LOG
        ...
        #ifndef DISABLE_RTKTUNING
        {
            //int64_t StartTime   = mSeekTimeUs;
            int64_t StartTime   = SeekTimeStartUs;
            int64_t Interval    = SeekTimeIntervalUs;
            int64_t MaxTime     = kHighWaterMarkSeekUs;
            int64_t MinTime     = kLowWaterMarkSeekUs;

            if (timeUs > (StartTime+Interval)) {
                mSeekThresholdUs = kLowWaterMarkSeekUs;
            } else if (timeUs < StartTime) {
                ALOGW("[%s %d] BUG!!! VideoPTS < StartTime (%.3f < %.3f)",__FUNCTION__,__LINE__,
                    (float)timeUs/1E6,(float)StartTime/1E6);
                mSeekThresholdUs = kLowWaterMarkSeekUs;
            } else {
                float scale = (float)(timeUs-StartTime)/Interval;
                mSeekThresholdUs = MaxTime - ((float)(MaxTime - MinTime) * scale);
                //ALOGI("[%s %d] mSeekThresholdUs = %.1f ms",__FUNCTION__,__LINE__,(float)mSeekThresholdUs/1E3);
            }
        }

        int64_t SeekThreshold = mSeekThresholdUs;
        if(isStreamingHTTP()) SeekThreshold *= 2;

        #else
        int64_t SeekThreshold  = 500000ll;
        #endif /* End of DISABLE_RTKTUNING */

        if (latenessUs > SeekThreshold //if (latenessUs > 500000ll
                && mAudioPlayer != NULL
...
void AwesomePlayer::postVideoEvent_l(int64_t delayUs) {
    ...
    #else
    if(delayUs >= 0){
        mQueue.postEventWithDelay(mVideoEvent, delayUs < 0 ? 5000 : delayUs);
    }else{
        if(mdcrt_fd != -1){
            int key = -1;
            int err = ioctl(mdcrt_fd, DCRT_CMD_WAIT_VSYNC, &key);

            if(err != 0)
            ALOGE("AwesomePlayer : %s : %d  err=%d key=%x\n", __FUNCTION__, __LINE__, err,key);

            mQueue.postEventWithDelay(mVideoEvent, 0);

        }else{
            ALOGE("AwesomePlayer : %s : %d dcrt_fd = %d Error!", __FUNCTION__, __LINE__,mdcrt_fd);

            mQueue.postEventWithDelay(mVideoEvent, 5000 );
...
void AwesomePlayer::postBufferingEvent_l() {
    ...
    /* PLAYING:
     *      When the Cache Duration is too small,
     *      we need more frequent checking adequacy of the cache!
     *        ^      ____
     * Post   |     /
     * Event  |    /
     * Delay  |___/
     ...
     * NOT PLAYING:
     *      When the Cache Duration is too small,
     *      we need more time to capture internet data.
     *        ^___
     * Post   |   \
     * Event  |    \
     * Delay  |     \___
     ...
     *         Cache Duration
     */
    static const int64_t MaxDelayUs = 1000000ll;
    static const int64_t MinDelayUs =  100000ll;
    int64_t DelayTimeUs = MinDelayUs;
    int64_t cachedDurationUs = 0;
    bool eos;
    if (getCachedDuration_l(&cachedDurationUs, &eos)) {
        if (cachedDurationUs > kHighWaterMarkUs) {
            DelayTimeUs = (mFlags & PLAYING)? MaxDelayUs : MinDelayUs;
        } else if (cachedDurationUs < kLowWaterMarkUs) {
            DelayTimeUs = (mFlags & PLAYING)? MinDelayUs : MaxDelayUs;
        } else {
            float scale = (float)(cachedDurationUs-kLowWaterMarkUs)/(kHighWaterMarkUs-kLowWaterMarkUs);
            if (mFlags & PLAYING) {
                DelayTimeUs = MinDelayUs + ((float) (MaxDelayUs-MinDelayUs) * scale);
            } else {
                DelayTimeUs = MaxDelayUs - ((float) (MaxDelayUs-MinDelayUs) * scale);
            }
        }
    } else {
        DelayTimeUs = MaxDelayUs;
    }

    #if 0 // FOR TEST
    DelayTimeUs = MinDelayUs;
    #endif

    if (!(mFlags & (PLAYING|AUDIO_AT_EOS|VIDEO_AT_EOS))) {
        ALOGI("[%s %d] Cach duration %.3f secs postDelay = %.1f ms",__FUNCTION__,__LINE__
            ,(float)cachedDurationUs/1E6,(float)DelayTimeUs/1E3);
    }

    mQueue.postEventWithDelay(mBufferingEvent, DelayTimeUs);
    #else
...
status_t AwesomePlayer::getTrackInfo(Parcel *reply) const {
    ...
    for (size_t i = 0; i < mExtractor->countTracks(); ++i) {
        ...
        reply->writeString16(String16(mime.string()));
