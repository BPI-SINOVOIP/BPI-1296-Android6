...
void OMX::setSideBand(node_id node, bool enableSideBand) {
    ALOGV("\033[0;32 @@@ In %s:%d enableSideBand:%d #### \033[m", __FUNCTION__, __LINE__, enableSideBand);
    return findInstance(node)->setSideBand(enableSideBand);
}

bool OMX::isSideBand(node_id node) {
    OMXNodeInstance *instance = findInstance(node);
    ALOGV("\033[0;32 @@@ In %s:%d isSideBand:%d #### \033[m", __FUNCTION__, __LINE__, (instance == NULL ? false : instance->isSideBand()));
    return (instance == NULL ? false : instance->isSideBand());
}
