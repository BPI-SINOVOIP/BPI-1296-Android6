...
struct BufferMeta {
    ...
    BufferMeta(size_t size, bool is_SideBand = false)
        : mSize(size),
          mIsSideBand(is_SideBand),
          mIsBackup(false) {
    }
...
status_t OMXNodeInstance::getParameter(
    ...
    if (index == OMX_IndexParamVideoInputPhysical)
    {
        ...
        checkHeader = findBufferHeader(inputHeader->nPortIndex);
        if (checkHeader != NULL )
        {
...
status_t OMXNodeInstance::allocateBuffer(
    ...
    BufferMeta *buffer_meta = new BufferMeta(size, mIsSideBand);
...
#if 1 /* [DHCFAE-1829] Avoid waiting for fence timeout when burn-in case */
        int max_loop = 30;
        status_t res;
        do {
            res = fence->wait(IOMX::kFenceTimeoutMs);
            if (res != OK)
                ALOGW("[%s] waiting for fence %d (%d)", __FUNCTION__, fenceFd, max_loop);
    ...
    *shareFd = 0;
    ...
    if (checkHeader != NULL )
    {
        ...
        if (rtkOmx != NULL)
        {
            ...
            ALOGD("\033[0;32m #### In %s:%d Get OMX_PLUS_EXTRADATATYPE, nPortIndex:%x, ionFd:%d, shareFd:%d nAddress:%llx newFd:%d enableCrypto:%d ####\033[m", __FUNCTION__, __LINE__, inputHeader->nPortIndex, rtkOmx->ionFd, inputHeader->shareFd, inputHeader->nAddress, *shareFd, inputHeader->enableCrypto);
        }
        else
        {
            inputHeader->nAddress = 0;
...
void OMXNodeInstance::setSideBand(bool enableSideBand)
{
    mIsSideBand = enableSideBand;
}
