...
status_t BufferQueueProducer::setBufferCount(int bufferCount) {
    ...
    { // Autolock scope
        ...
        mCore->mDequeueCondition.broadcast();
        if (listener != NULL)
            listener->onProducerNotify(BUFFER_COUNT, mCore->mOverrideMaxBufferCount);
    } // Autolock scope
...
#ifdef REVERT_0DE7EA7
        // Look for a free buffer to give to the client
        *found = BufferQueueCore::INVALID_BUFFER_SLOT;
#endif /* End of REVERT_0DE7EA7 */
        int dequeuedCount = 0;
        int acquiredCount = 0;
...
void BufferQueueProducer::cancelBuffer(int slot, const sp<Fence>& fence) {
    ...
    if (mSlots[slot].mFence != Fence::NO_FENCE && mSlots[slot].mFence > 0) {
        if (fence.get()) {
            sp<Fence> mergedFence = Fence::merge(
                    String8::format("%.28s:%d", mCore->mConsumerName.string(), slot),
                    mSlots[slot].mFence, fence);
            if (!mergedFence.get()) {
                BQ_LOGE("failed to merge release fences");
                mSlots[slot].mFence = fence;
            } else
                mSlots[slot].mFence = mergedFence;
        } else
            BQ_LOGV("[%s] %.28s:%d keep the fence.", __FUNCTION__, mCore->mConsumerName.string(), slot);
    } else
    mSlots[slot].mFence = fence;
    mCore->mDequeueCondition.broadcast();
#ifndef REVERT_0DE7EA7
...
status_t BufferQueueProducer::connect(const sp<IProducerListener>& listener,
    ...
    ATRACE_CALL();
    Mutex::Autolock lock(mCore->mMutex);
    sp<IConsumerListener> consumerListener = mCore->mConsumerListener;
    mConsumerName = mCore->mConsumerName;
