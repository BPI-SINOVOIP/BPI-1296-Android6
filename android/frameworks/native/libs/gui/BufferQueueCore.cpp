...
void BufferQueueCore::freeBufferLocked(int slot) {
    ...
    if (mSlots[slot].mFence != Fence::NO_FENCE && mSlots[slot].mFence > 0) {
        BQ_LOGI("[%s] fence wait        (slot: %d)", __FUNCTION__, slot);
        if (mConsumerListener != NULL)
            mConsumerListener->onProducerNotify(ProducerNotifyDefs::BUFFER_RELEASING, 0);
        mSlots[slot].mFence->waitForever(mConsumerName.string());
    } else if (mSlots[slot].mBufferState == BufferSlot::ACQUIRED) {
        BQ_LOGI("[%s] (slot: %d)", __FUNCTION__, slot);
        if (mConsumerListener != NULL)
            mConsumerListener->onProducerNotify(ProducerNotifyDefs::BUFFER_RELEASING, 1);
    }
#else
    if (    (mSlots[slot].mFence != Fence::NO_FENCE && mSlots[slot].mFence > 0) ||
    ...
    if (mConnectedApi != NATIVE_WINDOW_API_EGL) {
