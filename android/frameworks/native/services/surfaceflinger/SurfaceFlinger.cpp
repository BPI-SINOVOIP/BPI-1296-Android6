        ...
        ,mBootLogoDelayNs(0)
{
    ...
    property_get("persist.rtk.sf.bootlogo_delayMs", value, "0");
    mBootLogoDelayNs = atoi(value);
    mBootLogoDelayNs *= 1000000;
...
#else
    {
        class MessageStopBootAnimation : public MessageBase {
            public:
            MessageStopBootAnimation(){}
        ...
        sp<MessageBase> msg = new MessageStopBootAnimation();
        postMessageAsync(msg, mBootLogoDelayNs);
    }
#endif
}
            ...
            if (id >= 0) {
                ...
                for (size_t i=0 ; cur!=end && i<count ; ++i, ++cur) {
                    ...
                    if (Layer::eRTKVideoFlagsMark & layer->mRTKVideoFlags) {
                        ...
                        (*cur).setHints(HWC_HINT_RTK_SURFACEFLINGER2HAL_VIDEO_LAYER);
...
#ifdef SUPPORT_BOOTANIMATION_LOCK_DISPLAY_STACK
#   define BOOTANIMATION_LOCK_STACK                 (-2U)
#   define BOOTANIMATION_RESTORE_STACK              (-1U)
            if (disp.layerStack == BOOTANIMATION_LOCK_STACK) {
                static int store_layerStack = 0;
                int bootanim_exit = 0;
                char value[PROPERTY_VALUE_MAX];
                property_get("service.bootanim.exit", value, "0");
                bootanim_exit = atoi(value);
                if (s.layerStack == BOOTANIMATION_RESTORE_STACK) {
                    ALOGD("SF : restore layer stack %d", store_layerStack);
                    disp.layerStack = store_layerStack;
                    flags |= eDisplayTransactionNeeded;
                } else if (bootanim_exit) {
                    ALOGD("SF : set layer stack %d at boot exit", s.layerStack);
                    disp.layerStack = s.layerStack;
                    flags |= eDisplayTransactionNeeded;
                } else {
                    ALOGD("SF : store layer stack %d (%d)", s.layerStack, store_layerStack);
                    store_layerStack = s.layerStack;
                }
            } else
#endif /* SUPPORT_BOOTANIMATION_LOCK_DISPLAY_STACK */
