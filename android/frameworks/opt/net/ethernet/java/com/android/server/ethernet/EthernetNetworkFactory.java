...
class EthernetNetworkFactory {
        ...
        Thread dhcpThread = new Thread(new Runnable() {
            public void run() {
                ...
                if (config.getIpAssignment() == IpAssignment.STATIC) {
                    ...
                    // noticing.
                    // Dhcp retry most 5 minutes to determined that there is no DHCP server.
                    final int retryTime = 5*60*1000;
                    long beginDhcpTime = System.currentTimeMillis();
                    while (true) {
                        if (!NetworkUtils.runDhcp(mIface, dhcpResults)) {
                            ...
                            NetworkUtils.stopDhcp(mIface);
                            long deltaTime = System.currentTimeMillis()-beginDhcpTime;
                            if (DBG) Log.i(TAG, "deltaTime="+deltaTime);
                            if(deltaTime>retryTime){
                                Log.w(TAG, "runDhcp reach retryTime:"+retryTime+", stop dhcp");
                                return;
                            } else {
                                Log.i(TAG, "retry runDhcp");
                            }
                        } else {
                            break;
                        }
                ...
                if(mode==1) { //SCREEN_OFF_SUSPEND_ON
                    ...
                    if(UbusRpc.isOpenWrt() && !UbusRpc.isOttWifi()){
                        try{
                            ...
                            Log.e(TAG, "Failed to setInterfaceDown" + e);
                        }
                    }
    ...
    private void PppoeStart(boolean up){
