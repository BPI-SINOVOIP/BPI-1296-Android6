...
class PppoeNetworkFactory {
    ...
    PppoeNetworkFactory(RemoteCallbackList<IEthernetServiceListener> listeners) {
    ...
    public synchronized void start(Context context, Handler target) {
        ...
        String ifaceName = Utils.getIface(mIfaceMatch).getDisplayName();
        InterfaceAddress ifaceAddr = Utils.getIfaceIp(mIfaceMatch);
        ...
        if(gw.length()>0)
            staticIpConfig.gateway = getInetAddress(gw);
        String dns1 = SystemProperties.get("net.pppoe.dns1", "");
        if(dns1.length()>0)
            staticIpConfig.dnsServers.add(getInetAddress(dns1));
        String dns2 = SystemProperties.get("net.pppoe.dns2", "");
        if(dns2.length()>0)
            staticIpConfig.dnsServers.add(getInetAddress(dns2));
        mLinkProperties = staticIpConfig.toLinkProperties(ifaceName);
        // The services we use.
