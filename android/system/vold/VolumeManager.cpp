...
int VolumeManager::SmbMountShare(const char* smbPath, const char* mountPath, const char* user, const char* passwd, const char* domain)
{
    int mount_ret = 0;
    char option[256];
    sprintf(option, "username=%s,password=%s,domain=%s,iocharset=utf8,mapchars,soft,nounix,noserverino,sec=ntlm,uid=%u,gid=%u,file_mode=0777,dir_mode=0777", user, passwd, domain,AID_MEDIA_RW,AID_MEDIA_RW);
