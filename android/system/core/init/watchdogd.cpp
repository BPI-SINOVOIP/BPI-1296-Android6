...
int watchdogd_main(int argc, char **argv) {
    int success = 0;
    int bIsShutdown = 0;
    char value0[PROP_VALUE_MAX];
    char value1[PROP_VALUE_MAX];
    char isReboot[PROP_VALUE_MAX];
    ...
    while (true) {
        ...
        if (success == 0)
        {
            if ((property_get("dev.bootcomplete", value0) && !strcmp("1", value0))
                || (property_get("sys.boot_completed", value1) && !strcmp("1", value1)))
            {
                success = 1;
            }
        }
        else
        {
            if (bIsShutdown == 0)
            {
                ...
                if (property_get("rtk.power.shutdown", isReboot) && !strcmp("1", isReboot))
                {
