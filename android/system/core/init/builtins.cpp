...
#if 1
static void do_remount_ro(const char *path)
{
    int i, ret;
    INFO("remount %s as read-only...\n", path);
    for(i = 0; i < 50; i++){
    ...
    if(ret == 0)
        ERROR("remount %s as read only success! retry:%d\n", path, i);
    else
        ERROR("remount %s as read only failed: %s\n", path, strerror(errno));
}

#define SYS_PARTITIONS  "/sys.part.rc"
static void remount_ro_normal(void)
{
    char dev[64];
    char path[64];
    FILE *fd = fopen(SYS_PARTITIONS, "r");
    if(fd == NULL){
        ERROR("unable to open %s: %s\n", SYS_PARTITIONS, strerror(errno));
        return;
    }
    while(fscanf(fd, "%s %s", dev, path) == 2){
        do_remount_ro(path);
    }
    fclose(fd);
}

static void stop_service(const char *name)
{
...
static void stop_android(void)
{
    ERROR("stopping android....\n");

    //stop android procedure from stop.c
    stop_service("zygote_secondary");
    stop_service("zygote");
    stop_service("surfaceflinger");
    stop_service("sideband");

    system("vdc volume shutdown");
    stop_service("vold");

    //set interface wlan0 down, to avoid reboot wifi failure.
    system("ndc interface setcfg wlan0 down");
    stop_service("netd");

    //stop services that access /data parition
    stop_service("media");
    stop_service("DvdPlayer");
    stop_service("keystore");
    stop_service("gatekeeperd");
    stop_service("dhcpcd_eth0");
    stop_service("dhcpcd_wlan0");
    stop_service("dhcpcd_p2p");
}
#endif
...
#if 1
    sync();
    INFO("do_powerctl: command:%s\n", command);
    if(strncmp(command, "reboot,recovery", 15) != 0) {
        INFO("stop android and remount ro before reboot\n");
        stop_android();
        remount_ro_normal();
    }
#endif
