...
public abstract class AbstractFBViewActivity extends Activity implements UiListener
{
    ...
    private int m_DebugLevel = 2;//0~3: less msg ~ most msg
    ...
    private Handler All_Dev_DataProvider_handler = new MyHandler();
    ...
    public static final int MOVIE_REQUEST_CODE = 0;
            ...
            else if (msg.what == UPDATE_ALL_DEV_LIST)
            {
                ...
                if (msg.arg1==1&& msg.obj!=null)
                {
                    if (mDataProvider.GetItemIndex((String)msg.obj)== -1)
                    {
                        ...
                        Log.d(TAG,"UPDATE_ALL_DEV_LIST[+] can not get:"+(String)msg.obj);
                        removeMessages(UPDATE_ALL_DEV_LIST);
                        sendMessageDelayed(Message.obtain(msg),100);
                    }
                    else
                    {
                        Log.d(TAG,"UPDATE_ALL_DEV_LIST[+] get:"+(String)msg.obj);
                    }
                }
                if (msg.arg1==-1&& msg.obj!=null)
                {
...
//                        Toast.makeText(AbstractFBViewActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
	...
	DataProviderHandler mDataProviderHandler = null;
	private int GotNewItem = 0;
	
	//loading icon
	public Handler loading_icon_system_handle    = null;
	AnimationDrawable ad = null;
	ImageView topLoadingIcon =null;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        if(intent!=null && intent.getAction()!=null && intent.getAction().equals(Intent.ACTION_GET_CONTENT))
    ...
    public boolean CurrentPathIsValid(String ChangeIOPath)
    {
        boolean rlt = false;
	    if (mBrowserType == MbwType.TYPE_DEVICE_USB)
        {
            ...
            {
                //ChangeIOPath may be file:///dev/null
    ...
    public void BackToMediaBrowserHome()
    {
...
//Don't ask stop query in some Browser type, because we need query work complete done for creating playlist(ex:USB, DB video....), should ask don't update forground list and UI instead. But
    ...
    private void onItemClickedPostAction(DataProviderStruct stru){
        ...
        switch (stru.GetFileTypeAt)
        {
            ...
            {
                String filePath = "file://" + stru.GetDataAt;
            ...
            case FileFilterType.DEVICE_FILE_VIDEO:
            {
                ...
                if(filePath.startsWith("http")==false)
                    filePath = "file://" + Uri.encode(filePath);
                //Log.d(TAG,"FILE["+filePath+"]"+"isFile="+file.isFile()+"exists="+file.exists()+"canRead="+file.canRead()+"isDir="+file.isDirectory());
                ...
                // Blocked untile MovieActivity returned.
                ...
                if (!SINGLE_MUSIC_PLAYBACK)
                {
                    ...
                    for (int i = 0; i <size ; i++)
                    {
                        if (mBrowserType == MbwType.TYPE_DEVICE_DMS)
                        {
                            ...
                            String decodeUrl = Uri.decode(playlist.get(i));
                            if (filePath.regionMatches(0,decodeUrl,0,filePath.length()))
                            {
                                positionInPlaylist = i;
                                break;
                            }
    ...
    private class OptionListViewItemClickListener implements OnItemClickListener
    {
        ...
        public void onItemClick(AdapterView<?> parent, View v, int position, long id)
	   	{	
			...
			{
                if(mPop.getItemof(position).equals("mb_select")) {
                    mPop.Hide();
                    Intent intent = new Intent();
...
//        int y = getResources().getDimensionPixelSize(R.dimen.RTK_option_y);
   ...
   /*	private class LoginClickListener implements OnClickListener
    ...
    public boolean onBackClicked()
    {
		...
		if (!retval)
		{
			if(mBrowserType == MbwType.TYPE_DEVICE_ALL||mBrowserType == MbwType.TYPE_DB_PHOTO||mBrowserType == MbwType.TYPE_DB_AUDIO||mBrowserType == MbwType.TYPE_DB_VIDEO)
			{
                /*
				//Should back to main view of file browser
				ComponentName componetName = new ComponentName("com.rtk.mediabrowser",
	    		            "com.rtk.mediabrowser.MediaBrowser");
				Intent intent = new Intent();
        		intent.setComponent(componetName);
    ...
    public boolean onPlayClicked()
    {
		...
		{
			String path;
            path    = mDataProvider.GetDataAt(mPathInfo.getLastLevelFocus());
            ...
            if (BD.getType() != BDParser.NOT_BD_DVD)
            {
                ...
                log("onPlayClicked playEntry="+playEntry);
                intent.setData(Uri.parse(playEntry));
...
//                log("onPlayClicked playlist_title=" + Arrays.toString(playlist_title.toArray()));
                ...
                int size = playlist.size();
                for (int i = 0; i < size; i++) {
                    if (playEntry.equals(playlist.get(i))) {
                        positionInPlaylist = i;
                        break;
                    }
                }
        ...
        switch(requestCode){
            case MOVIE_REQUEST_CODE:
                if(resultCode==Activity.RESULT_OK){
                    ...
                    if(mGenericContentViewFragment!=null){
                        String videoPath = (String)data.getExtra("StopPosition");
