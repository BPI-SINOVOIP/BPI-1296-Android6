...
public class DecodeActivity extends Activity implements SurfaceHolder.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mResol = bundle.getString("RESOLUTION");
    ...
    private void prepareIntent()
    {
        mHandler = new Handler(Looper.getMainLooper())
        {
            ...
            public void handleMessage(Message msg) {
                Log.d(TAG, "Received transcode idx " + msg.arg1 + " state " + msg.arg2 + " " + msg.obj);
            }
        };
    ...
    public int startAllTranscodeService() {
        ...
        try{
            ...
            if(mIdx[i] != -1)
            {
                mService.setDataSource(mIdx[i], mSourcePath[i], mTargetPath[i], mOutoutFormat, mTrackOutput);
                mService.setVideoParameter(mIdx[i], mResol, mFrameRate, mBitRate, mIFrameInterval, mDeinterlace, mBitrateMode, mProfile, mLevel);
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ShowTitle();    
	Log.d(TAG,"the progarme in this way");  
        prepareIntent();
