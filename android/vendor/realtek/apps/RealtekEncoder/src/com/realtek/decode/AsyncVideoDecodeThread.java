...
public class AsyncVideoDecodeThread extends Thread {
    ...
    public AsyncVideoDecodeThread(String path, MediaMuxer muxer, String frameRate, int bitRate, int frameInterval, 
        String resolution, int enableDeinterlace, int bitrateMode, int profile, int level) {
        this.videoPath = path;
        this.muxer = muxer;
    ...
    public void run() {
        ...
        for (int i = 0; i < extractor.getTrackCount(); i++) {
            ...
            if (mime.startsWith("video/")) {
                ...
                Log.d(TAG, "Get Width " + format.getInteger(MediaFormat.KEY_WIDTH) + " Height " + format.getInteger(MediaFormat.KEY_HEIGHT) + " from extractor");
                encode_format.setInteger(MediaFormat.KEY_COLOR_FORMAT, MediaCodecInfo.CodecCapabilities.COLOR_FormatSurface);
                Log.d(TAG, "Get setting with Width " + mWidth + " Height " + mHeight + " frameRate " + frameRate + " bitRate " + bitRate + " frameInterval " + frameInterval + " bitrateMode " + bitrateMode + " profile " + profile + " level " + level);
                if(bitRate == -1)
