...
public class TranscodeManager {
    ...
    public void setDataSource(String source, String target, String output, int track) {
        try {
            mTranscodeService.setDataSource(mTranscodeIndex, source, target, output, track);
        } catch (RemoteException ex) {
            Log.e(TAG, "setDataSource RemoteException: " + ex.toString() + ": " + ex.getMessage());
        }
    }

    public void setVideoParameter(String resolution, String frameRate, int bitRate, int iFrame, 
        int enableDeinterlace, int bitrateMode, int profile, int level) {
        try {
            mTranscodeService.setVideoParameter(mTranscodeIndex, resolution, frameRate, bitRate, iFrame, 
