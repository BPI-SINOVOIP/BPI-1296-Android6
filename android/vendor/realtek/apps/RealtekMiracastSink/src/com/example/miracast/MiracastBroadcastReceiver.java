		...
		Log.i(TAG, "p2p disabled");
            ...
            if (networkInfo.isConnected()) {
				...
				if ((lState == NetworkInfo.DetailedState.DISCONNECTED) || (lState == NetworkInfo.DetailedState.FAILED))
				{
					...
					//check /tmp/rtk_wfd.terminate exist to make sure rtk_wfd terminateed, timeout 5 secs.
					int iwait = 0;
					do {
						File f = new File("/tmp/rtk_wfd.terminate");
						if(f.exists()) {
							Log.i(TAG, "OK: rtk_wfd terminate.");
							break;
					...
					if (iwait>=50) {
						Log.i(TAG, "WARNING: rtk_wfd seems not terminate!");
					}
					//end of check
