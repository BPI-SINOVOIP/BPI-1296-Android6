...
public class DecodeActivity extends Activity implements SurfaceHolder.Callback, View.OnTouchListener {
    ...
    private static int MEDIAPLAYER = 0;
    ...
    private MediaPlayer mMediaPlayer[] = new MediaPlayer[MAX_GRID_NUM];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AUDIO_CLICK_OPTION = 0;//bundle.getInt("AUDIOCLICK");
        MEDIAPLAYER = bundle.getInt("MEDIAPLAYER");
...
//        SAMPLE = getResources().getStringArray(R.array.playList);
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ...
        if(surfaceCount == gridNum)
        {
            ...
            else
            {
                for(int i=0; i<MAX_VIDEO_OUT; i++)
                {
                    mMediaPlayer[i] = new MediaPlayer();
                    mMediaPlayer[i].setDisplay(mSurfaceHolder[i]);
                    //mMediaPlayer[i].useRTMediaPlayer(MediaPlayer.FORCE_RT_MEDIAPLAYER);
                    try
                    {
                        mMediaPlayer[i].setDataSource(SAMPLE[i]);
                    }
                    catch (IOException e1) {
                        e1.printStackTrace();
                    }
                    mMediaPlayer[i].prepareAsync();
                    mMediaPlayer[i].start();
    ...
    {        
        String s = "PIP Demo : Start content from " + (startIdx+1) + " to " + (startIdx+gridNum);
        //setTitle(s);
