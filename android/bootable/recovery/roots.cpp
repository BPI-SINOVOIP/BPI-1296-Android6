...
void dump_volume_table_debug()
{
    int i;
    LOGI("[OTA_D]recovery filesystem table\n");
    LOGI("=========================\n");
    for (i = 0; i < fstab->num_entries; ++i)
    {
        Volume* v = &fstab->recs[i];
...
int load_volume_table_byLayoutFile()
{
    ...
    printf ("\t (fstab->recs cnt:%d; fstab->num_entries:%d) \n", cnt, fstab->num_entries);
    if (fstab->num_entries != cnt)
        fstab->num_entries = cnt;
    ...
    printf ("\t (fstab->num_entries_addTmp:%d) \n", fstab->num_entries);
    dump_volume_table_debug();
    return 0;
}

int add_fstab_entry_wrapper(const char *mount_point, const char *fs_type,const char *blk_device){

    //check already has entrty
    Volume * v =  volume_for_path(mount_point);
    if(v != NULL) return 0;

    int ret = fs_mgr_add_entry(fstab, mount_point, fs_type, blk_device);
    if (ret < 0 )
    {
        LOGE("failed to add %s entry to fstab\n",mount_point);
        fs_mgr_free_fstab(fstab);
        fstab = NULL;
        return -1;
    }
...
int setup_install_mounts() {
    ...
    printf("[OTA_D]fstab->num_entries:%d\n", fstab->num_entries);
    dump_volume_table_debug();
    for (int i = 0; i < fstab->num_entries; ++i)
    {
        Volume* v = &fstab->recs[i];
        printf("Get v->fs_type:%s, v->mount_point:%s; dev:%s\n", v->fs_type,v->mount_point, v->blk_device);
        if (strcmp(v->mount_point, "/tmp") == 0 ||
            strcmp(v->mount_point, "/cache") == 0) {
            printf("[OTA_D]v->mount_point=/cache!!\n");
