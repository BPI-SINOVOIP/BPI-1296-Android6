...
int rtk_burn_bootcode_emmc(CRTKImgDesc* prtkimgdesc, ZipArchive* za, char* zipPath)
{
    ...
    if (ChipVer)
        snprintf(cmd, sizeof(cmd), "cd /tmp;/sbin/busybox unzip %s bootloader_lk.tar;rm -rf %s;mkdir -p %s;tar -xf %s.tar -C %s/", zipPath, BOOTCODE_TEMP, BOOTCODE_TEMP, BOOTCODE_TEMP, BOOTCODE_TEMP);
    else
