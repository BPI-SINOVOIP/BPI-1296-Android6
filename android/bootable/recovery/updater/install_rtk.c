...
int rtk_file_checksum(char* sBuf, unsigned int imglen, unsigned char *checksum)
{
    ...
    memcpy(pBuffVirt_src, sBuf, imglen);
    RTKIon_sync();
...
int add_firmware(char* str, CRTKImgDesc* prtkimg)
{
    ...
    if(strcmp(firmware_name, "audioKernel") == 0 && get_chip_rev_id() >= RTD129x_CHIP_REVISION_B00)
        strcat(filename, ".b");
...
int save_fwdesc_emmc(CRtkFWHeader* fw, CRTKImgDesc* prtkimg)
{
    ...
    install_info("version:0x%02x fw_count:%d, part_count:%d\r\n",fw->fw_tab.version , fw->fw_count, fw->part_count);
...
int rtk_burn_bootcode_emmc(CRTKImgDesc* prtkimgdesc, ZipArchive* za, char* zipPath)
{
    ...
    if (ChipVer)
        snprintf(cmd, sizeof(cmd), "cd /tmp;/sbin/busybox unzip %s bootloader_lk.tar;rm -rf %s;mkdir -p %s;tar -xf bootloader_lk.tar -C %s/", zipPath, BOOTCODE_TEMP, BOOTCODE_TEMP, BOOTCODE_TEMP);
    else
...
#else
            tmpBuffer = malloc(prtkimgdesc->fw[efwtype].img_size + 32);
...
#ifdef FWDES_DUMP
    dump_fw_desc_table(&(emmc_rtk_fw_head.fw_tab));
#endif
...
int Rtk_burn_fwdesc_nand(CRTKImgDesc* prtkimg, State* state)
{
...
#ifdef FWDES_DUMP
    dump_fw_desc_table(&(nand_rtk_fw_head.fw_tab));
#endif
    ...
    {
        install_log("part desc[%d]\r\n",i);
        dump_part_desc_entry_v2(&(nand_rtk_fw_head.part_desc[i]));
    }
    for(i=0; i<(int)nand_rtk_fw_head.fw_count; i++)
    {
        install_log("fw desc[%d]\r\n",i);
        dump_rtk_fw_desc_entry_v22(&(nand_rtk_fw_head.fw_desc[i]));
    }
#endif
    close(dev_fd);
