...
class EdifyGenerator(object):
  ...
  def Unmount(self, mount_point):
    ...
    if fstab:
      ...
      self.script.append('unmount("%s");' %
                         (p.mount_point))
      self.mounts.remove(p.mount_point)
  ...
  def UntarPackageDir(self, src, dst):
    """Untar a given tar file from the OTA package into the given
    destination directory."""
    self.script.append('package_extract_tarfile("%s", "%s");' % (src, dst))
  ...
  def WriteRawImage(self, mount_point, fn):
    ...
    if fstab:
      ...
      if partition_type == "MTD":
        ...
        self.script.append(
            'write_raw_image_emmc(package_extract_file("%(fn)s"), "%(device)s");'
            % args)
      else:
  ...
  def WriteRTKNandImage(self):
    """ Write RTK related image for Nand flash case. """
    self.script.append('write_raw_rtkimage_nand();')
