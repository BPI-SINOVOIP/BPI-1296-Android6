...
class VOWriteBack {
    public:
        ...
        typedef struct mixdd_info_s {
            bool        enable;
            int         width;
            int         height;
            uint32_t    format_usage;
        } mixdd_info_t;
        static void     lib_rtk_query_mixdd_info(mixdd_info_t * info);
        typedef enum mixer_e {
            MIXER1 = 0,
            MIXER2
        } mixer_t;
        static void     lib_rtk_query_mixer_size(mixer_t mixer, int * pWidth, int * pHeight);
    private:
        void *                          mVORPC;
