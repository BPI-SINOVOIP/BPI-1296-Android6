...
public:
    ...
    /**
     * @brief get the position and size of video display window
     * @parm  x         - the top left position (x-axis)
     *        y         - the top left position (y-axis)
     *        width     - the width of video display window
     *        height    - the height of video display window
     ...
     *         -1 - fail
     */
    virtual int getVideoWindow(int *x, int *y, int *width, int *height);
...
private:
	...
	{
		int numItems;
		unsigned int duration[500]; // MAX 500 titles
	};
...
private:
	...
	pthread_mutex_t	m_SetDataSourceLock;
