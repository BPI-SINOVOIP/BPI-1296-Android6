...
bool Is420(VO_STANDARD videosystem);
#endif
void updateDPTXSetting(int videoSystem, int videoStandard, int videoType);
void createDPInfoFrame(VIDEO_RPC_VOUT_CONFIG_HDMI_INFO_FRAME* infoFrame);
int DPTX_GetVideoFormat(int *len, struct HDMI_TvSystemAndStandard *pTvSS);
int DPTX_GetAutoVideoFormat(bool lowResfirst=false);
void DPTX_TurnOffVideo();
