...
public:
	...
	HRESULT SetTvSystem(
		...
		bool bUpdateVideo = true);
#endif
	HRESULT setTvResolution(ENUM_VIDEO_SYSTEM videoSystem, ENUM_VIDEO_STANDARD videoStandard, ENUM_VIDEO_SYSTEM dp_videoSystem = VIDEO_NTSC, ENUM_VIDEO_STANDARD dp_videoStandard = VIDEO_INTERLACED, int videoType = 0);
...
public:
	...
	HRESULT SetFormat3d(int format3d, float fps = 0);
#if IS_CHIP(SATURN)
    ...
    HRESULT QueryDisplayWinNew(VIDEO_RPC_VOUT_QUERY_DISP_WIN_IN disp_plane_id, VIDEO_RPC_VOUT_QUERY_DISP_WIN_OUT_NEW *out);
...
public:
    ...
    HRESULT SetHDMICVBSDisplayRatio(int ratio);
    HRESULT SetDPDisplayRatio(int ratio);
    HRESULT SetDPDisplayXYOffset(int x, int y);
	HRESULT SetDPDisplayBCHSSetting(VIDEO_RPC_VOUT_SET_LPORT_COLOR arg);
	HRESULT QueryConfigTVSystem(VIDEO_RPC_VOUT_CONFIG_TV_SYSTEM *config);
    HRESULT SetHDROff(int hdr_off);
    bool isCVBSOn();
