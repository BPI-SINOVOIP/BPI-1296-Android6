...
AR= $(MYAR) cq
RANLIB= $(MYRANLIB)
LIBNAME= $(GENERIC_LINUX_PATH)/lib/libRTK_lib.so
LIBNAMESTATIC= $(GENERIC_LINUX_PATH)/lib/libRTK_lib.a
...
all: $(LIBNAME) $(LIBNAMESTATIC)

$(LIBNAME) : $(OBJ_FILES) 
	$(SHOW_COMMAND)$(rm) $@
	#$(SHOW_COMMAND)$(AR) $@ $(OBJ_FILES)
	#$(SHOW_COMMAND)$(RANLIB) $@
	$(CC) -shared  -o $@ $(OBJ_FILES) -L$(GENERIC_LINUX_PATH)/lib -lpthread -lion -Wl,--no-undefined

$(LIBNAMESTATIC) : $(OBJ_FILES)
	$(SHOW_COMMAND)$(rm) $@
	$(SHOW_COMMAND)$(AR) $@ $(OBJ_FILES)
	$(SHOW_COMMAND)$(RANLIB) $@
#	$(CC) -o $@ $(OBJ_FILES) -L$(GENERIC_LINUX_PATH)/lib -lpthread -lion 

clean:
	$(SHOW_COMMAND)$(rm) $(OBJ_FILES) $(LIBNAME) $(LIBNAMESTATIC) core *~
