...
def CopyLargeFilesToOut(input_tmp, tree_subdir, output_zip=None):
  """Copies large files underneath tree_subdir/ in the input zip to the output
  zip.
  """
  dir_src = os.path.join(input_tmp, tree_subdir)
  if os.path.isdir(dir_src):
    for filename in os.listdir(dir_src):
      filepath = os.path.join(dir_src,filename)
      target_filename = os.path.join(tree_subdir,filename)
      if os.path.isfile(filepath):
        print "CopyLargeFilesToOut- check %s" % (filename, )
      if output_zip is not None:
          output_zip.write(filepath, target_filename)
...
PARTITION_TYPES = { "yaffs2": "MTD", "mtd": "MTD", "ubifs":"UBI",
                    "ext4": "EMMC", "emmc": "EMMC", "raw": "RAW" }
