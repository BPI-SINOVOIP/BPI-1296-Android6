...
int LayerBaseRPC::toAgentState(enum AgentType cmd)
{
    ...
    switch (cmd) {
        case AgentRun:
            ...
            h = VIDEO_RPC_ToAgent_Run_0((long *)&mInstance, &mClnt);
            break;
            }
        case AgentPause:
            {
            ATRACE_NAME("VIDEO_RPC_ToAgent_Pause_0");
            h = VIDEO_RPC_ToAgent_Pause_0((long *)&mInstance, &mClnt);
            break;
            }
        case AgentStop:
            {
            ATRACE_NAME("VIDEO_RPC_ToAgent_Stop_0");
            h = VIDEO_RPC_ToAgent_Stop_0((long *)&mInstance, &mClnt);
            break;
            }
        case AgentDestroy:
            {
            ATRACE_NAME("VIDEO_RPC_ToAgent_Destroy_0");
            h = VIDEO_RPC_ToAgent_Destroy_0((long *)&mInstance, &mClnt);
            break;
            }
        default:
