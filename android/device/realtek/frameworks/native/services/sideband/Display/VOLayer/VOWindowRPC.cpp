...
int VOWindowRPC::setFlags(uint32_t flags, uint32_t mask) {
    ...
    if (mask & MAXER_OWNER) {
        if (mFlags & MAXER_OWNER)
            mMixer = VO_MIXER_OWNER_MASK;
        else
            mMixer = VO_MIXER_DEFAULT;
    }
...
int VOWindowRPC::WindowReSize() {
    ...
    if (WIDTH(mCrop) == 0 || HEIGHT(mCrop) == 0) {
        ...
        ret = ConfigureDisplayWindowZoomWin((VO_VIDEO_PLANE) (mPlane | mMixer),
                zoomWin,cropZoom,zoomWin,borderColor,false);
    } else {
        struct VO_COLOR borderColor = {.c1=0, .c2=0, .c3=0, .isRGB=1};
        ret = ConfigureDisplayWindow((VO_VIDEO_PLANE) (mPlane | mMixer),
