...
public class RtkDPTxService extends IRtkDPTxService.Stub {
	...
	public int [] getVideoFormat() {
		synchronized(this) {
			int setupID = 0;
			int[] supportVideoFormat = new int[RtkTVSystem.TV_SYS_DP_NUM];
			RtkTVSystem[] videoFormat = _getVideoFormat();

			for(int i = 0; i < videoFormat.length; i++) {
				setupID = -1;
				
				if (videoFormat[i].intVideoSystem== RtkTVSystem.VIDEO_DP_720P_60HZ)
					setupID = RtkTVSystem.TV_SYS_DP_720P_60HZ;
				else if (videoFormat[i].intVideoSystem == RtkTVSystem.VIDEO_DP_1080P_60HZ)
					setupID = RtkTVSystem.TV_SYS_DP_1080P_60HZ;
				else if (videoFormat[i].intVideoSystem == RtkTVSystem.VIDEO_DP_2160P_30HZ)
					setupID = RtkTVSystem.TV_SYS_DP_2160P_30HZ;
	...
	private int changeTVSystem(int tvSystem) {
		synchronized(this) {
			...
			nativeSetTVSystem(intSetTVsystem);
