    ...
    if (!mbrData.ReadMBRData((string) device)) {
        ...
        switch (mbrData.GetValidity()) {
            case mbr:
                ...
                break;
            case gpt:
            case hybrid: //Mac OS uses GPT for HFS+ partitions, but hybrid for exFAT/FAT.
                gptData.JustLooking();
                if (!gptData.LoadPartitions((string) device)) {
                    cerr << "Failed to read GPT" << endl;
                    return 9;
                ...
                for (int i = 0; i < numParts; i++) {
                    ...
                    if (partData.GetFirstLBA() > 0) {
                        if(partData.GetHexType() == 0xEF00){
                            //The EFI system partition is hidden in system, as same as Mac and Linux. 
                            continue;
                        }
