					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		wpa_supplicant_deauthenticate(wpa_s,
                ...
                /* 20150317 + Janet: bug 64272, To avoid PBC Overlap, it must remove sr=1 after disconnect */
                struct wpa_bss *bss, *n; int i=0;
                dl_list_for_each_safe(bss, n, &wpa_s->bss, struct wpa_bss, list) {
                        ...
                        if (wps_is_selected_pbc_registrar(ie)) {
                                wpa_printf(MSG_DEBUG, "Avoid PBC overlap, remove SR=1 bss when DISCONNECT: "MACSTR, MAC2STR(bss->bssid));
