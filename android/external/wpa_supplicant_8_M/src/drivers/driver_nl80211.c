	...
	struct wpa_driver_nl80211_data *drv, const char *ifname)
{
	...
	if (os_strcmp(drv->first_bss->ifname, ifname) == 0) {
		...
		/* DHCKYLIN-823: nl80211: Improve condition checking when receiving a RTM_NEWLINK event. Edward */
		if ((!drv->if_removed) || (!linux_iface_up(drv->global->ioctl_sock, ifname)))
			return;
